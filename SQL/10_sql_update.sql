...
CREATE TABLE [dbo].[pledge_ext](
	...
	[rbpoints] [int] NOT NULL DEFAULT ((0)),
	[rbpoints_rep] [int] NOT NULL DEFAULT ((0)),
...
CREATE TABLE [ItemDelivery] (
  ...
  [augmenation] int DEFAULT (0) NOT NULL,
  [auction] int DEFAULT (0) NOT NULL,
  [sold_id] int DEFAULT (0) NOT NULL,
...
CREATE PROCEDURE lin_ItemDeliveryAddJob
(
	@charId as INT,
	@itemId as INT,
	@itemAmount as INT,
...
SET NOCOUNT on;
BEGIN
	INSERT INTO [ItemDelivery] ([char_id], [item_id], [item_amount], [enchant], [augmenation], [auction], [sold_id], [sold_amount], [buyer]) 
	VALUES (@charId, @itemId, @itemAmount, @enchant, @augment, @auction, @soldId, @soldAmount, @buyer)
END
...
SET @auctionId = 0
INSERT INTO [Auction] ( seller_id, seller_name, item_id, amount, enchant, augmentation, priceid, price, expire_time ) VALUES (@sellerId, @sellerName, @itemId, @amount, @enchant, @augmentation, @priceid, @price, @expireTime)
IF (@@error = 0)  
BEGIN  
...
BEGIN
	...
	BEGIN
		UPDATE [BannedHardwareId] SET [account]=@account, [gm] = @gm, [timeout] = @timeout, [times] = @times WHERE [hardware_id] = @hwid
	END
	ELSE
	BEGIN
		INSERT INTO [BannedHardwareId] ([hardware_id], [account], [gm], [timeout], [times]) VALUES (@hwid, @account, @gm, @timeout, @times)
	END
END
go
...
-- Captcha System
ALTER TABLE dbo.user_data ADD
	captcha_mobs int NOT NULL DEFAULT 0,
	captcha_times int NOT NULL DEFAULT 0,
	captcha_period int NOT NULL DEFAULT 0,
	captchaQuestion int NOT NULL DEFAULT 0,
	captchaUniqueID int NOT NULL DEFAULT 0,
	captchaJail int NOT NULL DEFAULT 0
GO

-- Jail System
ALTER TABLE dbo.user_data ADD
	jailtimeout int NOT NULL DEFAULT 0
GO

-- Captcha Report System
ALTER TABLE dbo.user_data ADD
	captcha_reports int NOT NULL DEFAULT 0
GO

-- Event Stats
ALTER TABLE dbo.user_data ADD
	event_kills int NOT NULL DEFAULT 0,
	event_deaths int NOT NULL DEFAULT 0
GO

/****** Object:  Table [dbo].[offline_shop_items]    Script Date: 10/09/2017 16:53:45 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE TABLE [dbo].[offline_shop_items](
	[shop_item_id] [int] IDENTITY(1,1) NOT NULL,
	[shop_item] [int] NOT NULL,
	[shop_item_count] [bigint] NOT NULL,
	[shop_item_price] [bigint] NOT NULL,
	[shop_item_shop_id] [int] NOT NULL,
	[enchant] [int] NOT NULL,
	[item_class_id] [int] NOT NULL
 CONSTRAINT [PK_offline_shop_items] PRIMARY KEY CLUSTERED 
(
	[shop_item_id] ASC
)WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON) ON [PRIMARY]
) ON [PRIMARY]
GO
/****** Object:  Table [dbo].[offline_shop]    Script Date: 10/09/2017 16:53:45 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_PADDING ON
GO
CREATE TABLE [dbo].[offline_shop](
	[shop_id] [int] IDENTITY(1,1) NOT NULL,
	[shop_text] [varchar](255) NOT NULL,
	[shop_type] [tinyint] NOT NULL,
	[shop_owner_id] [int] NOT NULL,
 CONSTRAINT [PK_offline_shop] PRIMARY KEY CLUSTERED 
(
	[shop_id] ASC
)WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON) ON [PRIMARY]
) ON [PRIMARY]
...
/****** Object:  StoredProcedure [dbo].[lin_UpdateOfflineShopItemCount]    Script Date: 09/24/2019 02:58:06 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[lin_UpdateOfflineShopItemCount]
(
	@shop_owner_id	INT,
	@shop_item		INT,
	@item_count		INT,
...
BEGIN
	...
	DECLARE @shop_item_id INT
	DECLARE @new_item_count INT
	
	IF @nSell = 0
	BEGIN
		
		SELECT @shop_item_id = shop_item_id FROM offline_shop_items WHERE shop_item_shop_id = (SELECT shop_id FROM offline_shop WHERE shop_owner_id = @shop_owner_id) AND shop_item = @shop_item
			
		UPDATE offline_shop_items SET shop_item_count = shop_item_count - @item_count WHERE shop_item_id = @shop_item_id

		SELECT @new_item_count = shop_item_count FROM offline_shop_items WHERE shop_item_id = @shop_item_id

		IF @new_item_count <= 0
		BEGIN
			DELETE FROM offline_shop_items WHERE shop_item_id = @shop_item_id
		END
	END
	ELSE
	BEGIN
		SELECT @shop_item_id = shop_item_id FROM offline_shop_items WHERE shop_item_shop_id = (SELECT shop_id FROM offline_shop WHERE shop_owner_id = @shop_owner_id) AND item_class_id = @shop_item
...
END
GO
/****** Object:  StoredProcedure [dbo].[lin_AddOfflineShopItem]    Script Date: 10/09/2017 16:53:46 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[lin_AddOfflineShopItem]
(
	@shop_id			INT,
	@item_type			INT,
	@item_count			BIGINT,
...
BEGIN
	...
	INSERT INTO offline_shop_items (shop_item, shop_item_count, shop_item_price, shop_item_shop_id, enchant, item_class_id) VALUES(@item_type, @item_count, @item_price, @shop_id, @enchant, @item_class_id )
	SELECT @@IDENTITY
END
GO
...
/****** Object:  Table [dbo].[offline_shop_buff]    Script Date: 10/09/2017 16:53:45 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_PADDING ON
GO
CREATE TABLE [dbo].[offline_shop_buff](
	[shop_id] [int] IDENTITY(1,1) NOT NULL,
	[shop_owner_id] [int] NOT NULL,
	[shop_text] [varchar](255) NOT NULL,
	[price] [int] NOT NULL
 CONSTRAINT [PK_offline_shop_buff] PRIMARY KEY CLUSTERED 
(
	[shop_id] ASC
)WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON) ON [PRIMARY]
) ON [PRIMARY]
...
ALTER  TABLE offline_shop add 
	shop_locX int NOT NULL DEFAULT 0
go

ALTER  TABLE offline_shop add 
	shop_locY int NOT NULL DEFAULT 0
go

ALTER  TABLE offline_shop add 
	shop_locZ int NOT NULL DEFAULT 0
go
...
ALTER  TABLE offline_shop_buff add 
	shop_locX int NOT NULL DEFAULT 0
go
...
USE [lin2world]
GO
/****** Object:  StoredProcedure [dbo].[lin_AddOfflineShop]    Script Date: 28/07/2023 08:26:56 a.m. ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[lin_AddOfflineShop]
(
	@shop_text		VARCHAR(255),
	@shop_type		TINYINT,
	@shop_owner		INT,
	@shop_locx		INT,
	@shop_locy		INT,
...
BEGIN
	...
	-- first check for and delete existing shop
	DECLARE @existing_shop_id INT
	SELECT @existing_shop_id = shop_id FROM offline_shop WHERE shop_owner_id = @shop_owner
	IF @@ROWCOUNT > 0
	BEGIN
		DELETE FROM offline_shop_items WHERE shop_item_shop_id = @existing_shop_id
		DELETE FROM offline_shop WHERE shop_id = @existing_shop_id
	END
	
	-- insert new shop
	INSERT INTO offline_shop (shop_text, shop_type, shop_owner_id, shop_locX, shop_locY, shop_locZ, Heading) VALUES(@shop_text, @shop_type, @shop_owner, @shop_locx, @shop_locy, @shop_locz, @shop_heading)
	SELECT @@IDENTITY
END



/****** Object:  StoredProcedure [dbo].[lin_AddOfflineBuffShop]    Script Date: 28/07/2023 08:27:32 a.m. ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[lin_AddOfflineBuffShop]
(
	@shop_text		VARCHAR(255),
	@shop_price		INT,
	@shop_owner		INT,
...
BEGIN
	...
	-- first check for and delete existing shop
	DECLARE @existing_shop_id INT
	SELECT @existing_shop_id = shop_id FROM offline_shop_buff WHERE shop_owner_id = @shop_owner
	IF @@ROWCOUNT > 0
	BEGIN
		DELETE FROM offline_shop_buff WHERE shop_id = @existing_shop_id
	END
	
	-- insert new shop
	INSERT INTO offline_shop_buff (shop_text, shop_owner_id, price, shop_locX, shop_locY, shop_locZ, Heading) VALUES(@shop_text, @shop_owner, @shop_price, @shop_locx, @shop_locy, @shop_locz, @shop_heading)
...
CREATE TABLE [VoteReward](
	[ip_addr] NVARCHAR(255) NOT NULL DEFAULT N'',
	[l2topzone] INT NOT NULL DEFAULT 0,
	[l2topzone_reward] INT NOT NULL DEFAULT 0,
	[l2hopzone] INT NOT NULL DEFAULT 0,
	[l2hopzone_reward] INT NOT NULL DEFAULT 0,
	[l2network] INT NOT NULL DEFAULT 0,
	[l2network_reward] INT NOT NULL DEFAULT 0,
	[l2jbrasil] INT NOT NULL DEFAULT 0,
	[l2jbrasil_reward] INT NOT NULL DEFAULT 0,
	[l2servers] INT NOT NULL DEFAULT 0,
	[l2servers_reward] INT NOT NULL DEFAULT 0,
	[l2mmotop] INT NOT NULL DEFAULT 0,
	[l2mmotop_reward] INT NOT NULL DEFAULT 0,		
	[lastchecked] INT NOT NULL DEFAULT 0,	
	[hwid] NVARCHAR(255) NOT NULL DEFAULT N''
	 
)
GO

/****** Object:  StoredProcedure [lin_UpdateVoteReward]    Script Date: 13/05/2019 16:53:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[lin_UpdateVoteReward]
(
	/*@shop_text		VARCHAR(255),
	@shop_item		INT,
	@item_count		INT,
	*/
	@ip_addr		VARCHAR(255),
	@l2topzone		INT,
	@l2topzone_reward		INT,
	@l2hopzone		INT,
	@l2hopzone_reward		INT,
	@l2network		INT,
	@l2network_reward		INT,
	@l2jbrasil		INT,
	@l2jbrasil_reward		INT,
	@l2servers		INT,
	@l2servers_reward		INT,
	@l2mmotop		INT,
	@l2mmotop_reward		INT,
	@lastchecked		INT,
	@hwid		VARCHAR(255)
...
BEGIN
	...
	--DECLARE @ip_address_count INT
	--SELECT @ip_address_count = COUNT(ip_addr) FROM VoteReward WHERE ip_addr = @ip_addr

	IF EXISTS(SELECT * FROM VoteReward WHERE ip_addr = @ip_addr)
	--IF @ip_address_count > 0
		BEGIN
			UPDATE VoteReward SET l2topzone=@l2topzone , l2topzone_reward=@l2topzone_reward, l2hopzone=@l2hopzone , l2hopzone_reward=@l2hopzone_reward , l2network=@l2network ,l2network_reward=@l2network_reward ,
			l2jbrasil=@l2jbrasil, l2jbrasil_reward=@l2jbrasil_reward, l2servers=@l2servers, l2servers_reward=@l2servers_reward, l2mmotop=@l2mmotop, l2mmotop_reward=@l2mmotop_reward, lastchecked=@lastchecked, hwid=@hwid
			WHERE ip_addr=@ip_addr
		END
	ELSE
		BEGIN
			INSERT INTO VoteReward (ip_addr, l2topzone, l2topzone_reward, l2hopzone, l2hopzone_reward, l2network, l2network_reward, l2jbrasil, l2jbrasil_reward, l2servers, l2servers_reward, l2mmotop, l2mmotop_reward, lastchecked, hwid)
			VALUES (@ip_addr, @l2topzone, @l2topzone_reward, @l2hopzone, @l2hopzone_reward, @l2network, @l2network_reward, @l2jbrasil, @l2jbrasil_reward, @l2servers, @l2servers_reward, @l2mmotop, @l2mmotop_reward, @lastchecked, @hwid)
		END
END
GO


--FIX EXPLOIT ENCHANT SKILL CUANDO SWHICHEAN
set ANSI_NULLS ON
set QUOTED_IDENTIFIER ON
GO
...
UPDATE user_skill SET skill_lev = @skill_level WHERE char_id = @char_id AND skill_id = @skill_id AND skill_lev > 99 AND  @skill_id < 100
UPDATE user_skill SET skill_lev = @skill_level WHERE char_id = @char_id AND skill_id = @skill_id AND skill_lev > 99 AND  @skill_id > 100
