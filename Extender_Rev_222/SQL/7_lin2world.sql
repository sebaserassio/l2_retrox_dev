...
select isnull( sum(amount) , 0) from user_item (nolock) where warehouse = 2 and char_id = @pledge_id and item_type = 54
GO
...
BEGIN
	...
	SET sociality = @sociality , used_sulffrage = @sulffrage , last_changed = CAST(@last_changed AS date)
	WHERE char_id = @char_id
END
ELSE
BEGIN
	INSERT INTO user_sociality (char_id, sociality , used_sulffrage , last_changed ) VALUES (@char_id, @sociality , @sulffrage , CAST(@last_changed AS date))
END
GO
...
UPDATE  time_data SET last_logout = CAST(@dtLastSaveDate AS date), used_sec = @nUsedSec WHERE account_id = @account_id 

IF (@@ROWCOUNT = 0)
BEGIN
	INSERT INTO time_data (account_id, last_logout, used_sec) VALUES (@account_id, CAST(@dtLastSaveDate AS date), @nUsedSec)    
END
GO
