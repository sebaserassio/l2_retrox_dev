...
int CParser::ContarCaracteres(wstring str, wstring caracter)
{
	int contador = 0;
	size_t pos = str.find_first_of(caracter);
	while(pos!=wstring::npos)
	{
		contador++;
		str.erase(pos, 1);
		pos = str.find_first_of(caracter);
	}
	return contador;
}

int CParser::BuscarCoincidencias(wstring str, wstring caracteres)
{
	int contador = 0;
	size_t pos = str.find(caracteres);
	while(pos!=wstring::npos)
	{
		contador++;
		str.erase(pos, caracteres.size());
		pos = str.find(caracteres);
	}
	return contador;
}
...
wstring CParser::Trim2(wstring str)
{
	size_t pos = str.find_first_of(L"\n\r\v ");
	while(pos!=wstring::npos)
	{
...
wstring CParser::GetBypassValueConEspacios(wstring wBypass, wstring wSub)
{
	wstring wRet;
	//Remove white spaces from bypass
	//wBypass = Trim(wBypass);
...
wstring Parser::ParseOptionStringConEspacios(wstring line, wstring option, wstring sDefault)
{
	wstring wRet;
	//Remove white spaces from bypass
	//wBypass = Trim(wBypass);
	...
	if(pos!=wstring::npos)
	{
		...
		wRet = line.substr(pos, (line.size() - pos-1));
	}
	return wRet;
}
