...
bool CSkillEffect_p_frenzy_activo::CheckPumpCondition(CCreature *pCreature)
{
	if(User *pUser = pCreature->GetUser())
	{
		return true;
...
void CSkillEffect_p_frenzy_activo::Pump(CCreature *pCreature, CSkillInfo *pSI)
{
	if(pCreature->ValidUser())
	{
		pCreature->pSD->pExData->SkillMod.nFrenzyActivo = TRUE;
	}
}


bool CSkillEffect_p_stop_exp::CheckPumpCondition(CCreature *pCreature)
{
	if(User *pUser = pCreature->GetUser())
	{
		return true;
...
void CSkillEffect_p_stop_exp::Pump(CCreature *pCreature, CSkillInfo *pSI)
{
	if(pCreature->ValidUser())
	{
		pCreature->pSD->pExData->SkillMod.stopExp = TRUE;
		pCreature->pSD->pExData->stopIncExp = TRUE;
	}
}

bool CSkillEffect_p_global_shout::CheckPumpCondition(CCreature *pCreature)
{
	if(User *pUser = pCreature->GetUser())
	{
		return true;
...
void CSkillEffect_p_global_shout::Pump(CCreature *pCreature, CSkillInfo *pSI)
{
	if(pCreature->ValidUser())
	{
		pCreature->pSD->pExData->SkillMod.globalShout = TRUE;
	}
}
...
bool CSkillEffect_p_pvp_physical_skill_dmg_bonus_by_weapon::CheckPumpCondition(CCreature *pCreature)
{
	if(pCreature->ValidCreature())
	{
		CreatureSharedData *pSD = pCreature->pSD;
...
void CSkillEffect_p_pvp_physical_skill_dmg_bonus_by_weapon::Pump(CCreature *pCreature, CSkillInfo *pSkillInfo)
{
	if(pCreature)
	{
		switch(this->ChangeType)
		...
		case PER:
			{
				double dPvp_physical_skill_dmg_bonus = this->DamageAmount;
				dPvp_physical_skill_dmg_bonus += 100;
				dPvp_physical_skill_dmg_bonus /= 100;
				dPvp_physical_skill_dmg_bonus *= pCreature->pvp_physical_skill_dmg_bonus;
				pCreature->pvp_physical_skill_dmg_bonus = dPvp_physical_skill_dmg_bonus;
				break;
			}
		default:
			{
...
void CSkillEffect_p_anti_buff::Pump(CCreature *pCreature, CSkillInfo *pSI)
{
	if(pCreature->ValidCreature())
	{
		...
		{
			pCreature->pSD->pExData->SkillMod.nAntiBuff = 3;
		}else if(dSelf == 4)
		{
			pCreature->pSD->pExData->SkillMod.nAntiBuff = 4;
		}else
		{
...
bool CSkillEffect_p_defence_attribute_by_class::CheckPumpCondition(CCreature *pCreature)
{
	if(pCreature)
	{
		if(pCreature->pSD->nClass == 1)
		{
			return true;
		}
	}
...
void CSkillEffect_p_defence_attribute_by_class::Pump(CCreature* pCreature, CSkillInfo* pSI)
{
	if(pCreature->ValidCreature())
	{
		if(attribute >= 0 && attribute < 34)
...
bool CSkillEffect_p_custom_conversion::CheckPumpCondition(CCreature *pCreature)
{
	return true;
};

void CSkillEffect_p_custom_conversion::Pump(CCreature *pCreature, CSkillInfo *pSI)
{
	if(pCreature->ValidCreature())
	{
		double nPercent = static_cast<double>(dPercent);

		nPercent /= 100;

		pCreature->pSD->MaxHP *= nPercent;
	}
}



bool CSkillEffect_p_unequip_item::CheckPumpCondition(CCreature *pCreature)
{
	return true;
};

void CSkillEffect_p_unequip_item::Pump(CCreature *pCreature, CSkillInfo *pSI)
{
	if(pCreature->ValidCreature())
	{
		g_Log.Add(CLog::Blue, "[%s] NO SE PUEDE UTILIZAR, SE ENCUENTRA BLOQUEADO EL THREAD ENTONCES NO PUEDE  BAJAR NINGUN ITEM EN ESTA SITUACION AL EQUIPAR UN ITEM, SEGURAMENTE SE BLOQUEA EL INVENTARY LOCK", __FUNCTION__);
	}
}
