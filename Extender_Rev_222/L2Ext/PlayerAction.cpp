...
extern UINT m_SendMsgPickUp;
extern UINT m_AnnouncePickUpMsgItemId;
extern wstring m_AnnounceOnPickUpClan;
extern wstring m_AnnounceOnPickUpNormal;
...
void PlayerAction::ShowHTML(User *pUser, const WCHAR *wName, const WCHAR *wHtml, UINT itemClassId)
{
    ...
    if( pUser->SafeCastUser())
    {
        ...
        // Interceptar SOLO el HTML del ranking de clanes
        if(wName && (_wcsicmp(wName, L"voni001.htm") == 0))
        {
            g_AutoAnnounce.ShowClanRanking(pUser);
            unguard;
            return;
        }


        // Copia el HTML en una cadena modificable
        std::wstring html(wHtml);

        // Creamos un vector con los patrones de búsqueda:
        // Incluye comandos de teletransporte, menú y teleport_request.
        std::vector<std::wstring> searchPatterns;
        searchPatterns.push_back(L"player_teleport?name=");
        searchPatterns.push_back(L"menu_select?");
        searchPatterns.push_back(L"teleport_request");
        searchPatterns.push_back(L"deposit");
        searchPatterns.push_back(L"withdraw");
        searchPatterns.push_back(L"deposit_pledge");
        searchPatterns.push_back(L"withdraw_pledge");
		

        // Recorremos cada patrón de búsqueda
        for (size_t i = 0; i < searchPatterns.size(); i++)
        {
            std::wstring pattern = searchPatterns[i];
            size_t pos = 0;
            while ((pos = html.find(pattern, pos)) != std::wstring::npos)
            {
                // Se determina el final del comando buscando una comilla (") o un espacio
                size_t endPos = html.find(L"\"", pos);
                if (endPos == std::wstring::npos)
                {
                    endPos = html.find(L" ", pos);
                }
                if (endPos == std::wstring::npos)
                {
                    endPos = html.size();
                ...
                // Extrae el comando para verificar si ya contiene el parámetro rnd
                std::wstring command = html.substr(pos, endPos - pos);
                if (command.find(L"rnd=") == std::wstring::npos)
                {
                    // Genera un número aleatorio y conviértelo a cadena
                    int rndValue = std::rand();
                    wchar_t buf[32];
                    swprintf(buf, 32, L"%d", rndValue);

                    // Define el separador: si el comando no contiene "?" se usa "?", de lo contrario "&"
                    ...
                    std::wstring insertion = separator + L"rnd=";
                    insertion += buf;

                    // Inserta el parámetro justo antes del final del comando
                    html.insert(endPos, insertion);

                    // Actualiza la posición para continuar la búsqueda después del comando modificado
                    pos = endPos + insertion.length();
                }
                else
                {
                    pos = endPos;
        ...
        // Registra el HTML modificado en el filtro y lo envía al cliente
        g_HtmlFilter.OnSendHtml(pUser, wName, html.c_str(), itemClassId);

        typedef void (*f)(User*, const WCHAR*, const WCHAR*, UINT);
        f(0x80A060L)(pUser, wName, html.c_str(), itemClassId);
    }
    unguard;
}
...
void PlayerAction::OnTimerExpired(User *pUser, int nTimerID)
{
	...
	if(nTimerID == 0)
	{
		if(pUser->ValidUser())
		{
			...
			if(pUser->pSD->nHero != 2 && pUser->pED->nInCustomHeroTime > time(0))
			{
				pUser->pSD->nHero = 2;
			...
			else if(pUser->pSD->nHero == 2 && pUser->pED->nInCustomHeroTime < time(0))
			{
				g_DB.RequestRemoveHero(pUser->nDBID);
			}
			...
			if(pUser->pED->shield_test_timestamp < time(0) && pUser->pED->shield_test_timestamp>0 && pUser->pED->shield_test_id > 0 )
			{
				pUser->pED->shield_test_timestamp = 0;
				pUser->pED->shield_test_id = 0;
				PlayerAction::OnTeleportToLocation(pUser, static_cast<int>(pUser->pSD->Pos.x), static_cast<int>(pUser->pSD->Pos.y), static_cast<int>(pUser->pSD->Pos.z), 0);
			}

			if(pUser->pED->weapon_test_timestamp < time(0) && pUser->pED->weapon_test_timestamp>0 && pUser->pED->weapon_test_id > 0 )
			{
				pUser->pED->weapon_test_timestamp = 0;
				pUser->pED->weapon_test_id = 0;
				PlayerAction::OnTeleportToLocation(pUser, static_cast<int>(pUser->pSD->Pos.x), static_cast<int>(pUser->pSD->Pos.y), static_cast<int>(pUser->pSD->Pos.z), 0);
			}
					...
					}else
					{
						...
						if((diffTime % 5) == 0 || diffTime < 5)
						{
							...
							//	pUser->SendSystemMessage(&msg);
								...
								announce = Utils::ReplaceString(announce, _T("$s1"), Utils::IntToWString(diffTime));
...
bool PlayerAction::UseItem(User *pUser, CItem *pItem, int nForce)
{
	if(pItem->IsValidItem() && pUser->ValidUser())
	{
		...
		//-------------------------------------PISAR JOYAS COSUTOM CON LAS UNICAS BOSS

		UINT BajarItem = 0;
		if(pItem->pSID->nItemID == 18210)	//baium_chief_horus
			BajarItem = 6658;	//ring_of_baium
		else if(pItem->pSID->nItemID == 18211)	//queen_ant_chief_brakki
			BajarItem = 6660;	//ring_of_queen_ant
		else if(pItem->pSID->nItemID == 18208)	//antaras_earring_of_aurakyria
			BajarItem = 6656;	//earring_of_antaras
		else if(pItem->pSID->nItemID == 18209)	//zaken_earring_of_golkonda
			BajarItem = 6659;	//earring_of_antaras

		if(pItem->pSID->nItemID == 6658)	//ring_of_baium
			BajarItem = 18210;	//baium_chief_horus
		else if(pItem->pSID->nItemID == 6660)	//ring_of_queen_ant
			BajarItem = 18211;	//queen_ant_chief_brakki
		else if(pItem->pSID->nItemID == 6656)	//earring_of_antaras
			BajarItem = 18208;	//antaras_earring_of_aurakyria
		else if(pItem->pSID->nItemID == 6659)	//earring_of_antaras
			BajarItem = 18209;	//zaken_earring_of_golkonda
			...
			for(int n=0;n<18;n++)
			{
				...
				if(nItemIndex > 0)
				{
					...
					if(pSID)
					{
						...
						{
							if(BajarItem == pSID->nItemID)
							{
								//g_Log.Add(CLog::Blue, "[%s] 55555555 pSID->nItemID[%d], n[%d]", __FUNCTION__, pSID->nItemID, n);
								pUser->UnEquipItem(n);
							}
						}
					}
...
bool PlayerAction::OnClassChange(User *pUser, int nClass)
{
	...
	if(bRet)
	{
		if(CCategoryDataDB::IsInCategory(nClass, CCategoryDataDB::MAGE_GROUP))
			pUser->pED->EsMago = 1;
...
void PlayerAction::OnPickUpItem(CInventory *pInventory, CItem *pItem, int nChangedType)
{
	...
	if(pItem->pSID->nItemID == m_AnnouncePickUpMsgItemId)
	{
		...
		if (pUser->ValidUser())
		{
			...
			wstring AnnounceOnPickUpClan =  m_AnnounceOnPickUpClan;
			wstring AnnounceOnPickUpNormal =  m_AnnounceOnPickUpNormal;
			wstring nClanName = L"";


			CPledge *pPledge = pOwner->GetPledge();
			...
			{
				wstringstream name;
				name << pOwner->pSD->wszName;
				wstring charname;
				name >> charname;

				wstring announce = AnnounceOnPickUpClan;

				size_t pos = announce.find(L"$clan");
				...
				CSystemMessage msg(m_SendMsgPickUp);
			...
			else
			{
				...
				wstring announce = AnnounceOnPickUpNormal;
		...
		if(pItem->IsValidItem())
		{
			if(pUser->ValidUser())
			{
				...
				if(pItem->nLifeTime != 0)
				{
					...
					pUser->SendLifeTimeMessageForItem(pItem);
				}

				if(pUser->pED->ultimoMultisell == 22222)		//nListID
				{
					if(pUser->pED->ultimoMultisellCantidad)
					{


//;{[Shield_of_Dragons];1};		27701
//{[hat_71];1};				11671
//{[tatto_Galaxy4];1}			12204

						int itemId = pItem->pSID->nItemID;

						if(itemId != 27701 && itemId != 11671 && itemId != 12204)
						{
							if(pItem->GetWeapon())
							{
								CItemEnchant::CustomEnchantItem(pUser, pItem, 12);
							}
							else
							{
								CItemEnchant::CustomEnchantItem(pUser, pItem, 8);
							}
						}

						pItem->nLifeTime = (time(0) + 172800);			//172.800 = 48 Horas (2 Dias)
						g_DB.RequestSaveItemDataEx(pUser->nDBID, pItem);
						pUser->SendLifeTimeMessageForItem(pItem);

						pUser->pED->ultimoMultisellCantidad--;
					}
				}
			}
		}
...
int PlayerAction::OnGetRelationTo(User *pUser, User *pAttacker)
{
	...
	if(pUser->ValidUser() && pAttacker->ValidUser())
	{
		...
		//Custom inCustomSpawnZone
		if(!pUser->IsMyParty(pAttacker))
		{
			if(pUser->pED->inCustomSpawnZone || pAttacker->pED->inCustomSpawnZone)
			{
				return 0x6C4;
			}
		}
...
bool PlayerAction::OnEnterWorld(User *pUser)
{
	if(pUser->ValidUser())
	{
		...
		pUser->SendSystemMessage(L"Powered by GX-Ext");
		...
		if(g_Captcha.IsTestHWID(pUser->pSocket->pED->longHWID.hash))
		{

		//	g_Log.Add(CLog::Error, "[%s] 111111111", __FUNCTION__);
			pUser->pED->PedirCaptcha = 1;
		}
		...
		g_DB.RequestLoadHeroTime(pUser->nDBID);
		...
		g_PvPRankEvent.EnterWorld(pUser);

		if(NoPermitirDosTanquesLogout)
		{
			pUser->pSocket->Close(L"NoPermitirDosTanquesLogout");
			//g_Log.Add(CLog::Error, "[%s] NoPermitirDosTanquesLogout[%S]", __FUNCTION__, pUser->pSD->wszName);
		}
		...
		{
			//g_Log.Add(CLog::Error, "[%s] count > g_MaximoDeTercerasLogout && !pUser->pSD->nBuilder[%S] [%d][%d] --- [%d][%d][%d][%d]-[%d][%d][%d][%d]", __FUNCTION__, pUser->pSD->wszName, count, g_MaximoDeTercerasLogout, pUser->pSD->nSubJobClass[0], pUser->pSD->nSubJobClass[1], pUser->pSD->nSubJobClass[2], pUser->pSD->nSubJobClass[3], pUser->pED->nCustomSubClass[0], pUser->pED->nCustomSubClass[1], pUser->pED->nCustomSubClass[2], pUser->pED->nCustomSubClass[3]);
			pUser->pSocket->Close(L"count > g_MaximoDeTercerasLogout && !pUser->pSD->nBuilder");
		}
...
bool PlayerAction::OnDie(User *pUser, CCreature *pEnemy)
{
	...
	if(pEnemy->ValidUser())
	{
		if(g_PvPRankEvent.IsActive() && pEnemy->SafeCastUser()->pED->inZonePvPRank)
		{
			g_PvPRankEvent.OnPvPKill(pEnemy->SafeCastUser(), pUser);
		}
		...
		if(!pEnemy->SafeCastUser()->pED->nNoSumarPVPniPk)
		{
			...
			//PvP Reward zone
			territoryId = pUser->pSD->pExData->territoryData.territoryId[PVP_REWARD_ZONE];
			if(territoryId != 0)
			{
				User *pEnemyUser = 0;
				if(pEnemy->ValidUser())
	...
	if(UD(pUser, pEnemy))
	{
		...
		if(User *pEnemyUser = pEnemy->GetResponsibleUser())
		{
			...
			httpSender.SendPvPEvent(pEnemy->pSD->wszName, pUser->pSD->wszName);
