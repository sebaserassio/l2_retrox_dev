...
namespace Utils
{
	...
	void BroadcastToAllUser_AnnounceDist(const WCHAR* wAnnounce, int dist, FVector pos);
	...
	wstring ReplaceStringD(wstring baseStr, wstring oldStr, double nNew, bool all);
	string ReplaceStringA(string wStr, string wOld, string wNew, bool all = false);
	string IntToString(int a);
	wstring IntToWString(int a);
	wstring RandomString(int len);
	std::vector<BYTE> HexToBytes(const wstring& hex);
	std::wstring FormatWithCommas(UINT value);



	int GetIDByClass(wstring NewClass);
	wstring GetClassByID(int nClass);
	int GetIDdeSegundaToTercera(int Tercera);


	std::wstring NormalizeWString(const std::wstring& input);
	bool ContainProhibitedPhrase(const std::wstring& input, const std::set<std::wstring>& prohibitedPhrases);
