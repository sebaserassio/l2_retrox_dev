...
void CAutoStart::LoadINI()
{
	...
	g_HookManager.WriteCall(0x6F0B12, NpcDie, 0);
}
...
void CAutoStart::NpcDie(UINT64 pLog, int type, const WCHAR *format)
{
	//typedef void (*f)(UINT64, int, const WCHAR*);
	//f(0x4AAAE0L)(pLog, type, format);
	g_Log.Add(CLog::Error, "NPC Removing DONE!!!");
