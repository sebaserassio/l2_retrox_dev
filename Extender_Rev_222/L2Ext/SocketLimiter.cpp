...
void SocketLimiter::UserSocketBadPacketSizeLog(LPVOID lpInstance, UINT logType, const WCHAR* format, UINT64 sockFunc, int packetSize)
{
	...
	if(g_LastInvalidPacketLog < currentTick)
	{
		...
		if(threadIndex < 32)
		{
			...
			if(pSocket && pSocket->ValidUserSocket())
			{
	...
	try
	{
		...
		if(pSocket->pED->onReadTimeout > currentTick || ((DWORD)(pSocket->pED->onReadTimeout - 30000) > currentTick))
		{
			...
			}else
			{
				...
				pSocket->Close(L"SocketLimiter::Read pSocket->pED->ioReadCount < g_IOReadPerSecond");
			}
		}else
		{
			//close socket
			...
			pSocket->Close(L"SocketLimiter::Read pSocket->pED->onReadTimeout > currentTick || ((DWORD)(pSocket->pED->onReadTimeout - 30000) > currentTick");
		}
	}
	catch(...)
	{
