...
User *CCreature::GetMaster()
{
	typedef User*(__fastcall *__GetMaster)(CCreature *);
...
void CCreature::AddItemToInventory(int itemId, int amount, bool isPick, int enchanted, int blessed, int eroded, UINT augmentation)
{
	if(User *pUser = this->SafeCastUser())
	{
		if( itemId == 57 ) //ADENA
		{
			if(pUser->inventory.GetAdenaAmount() >= 2000000000)
			{
				if(pUser->inventory.CheckAddable(9631, 30))
				{
					pUser->DeleteItemInInventory(57, 300000000);
					pUser->AddItemToInventory(9631, 30);
					g_Logger.Add(L"User[%s] exchange adena[%d] into adena coin.", pUser->pSD->wszName, 300000000);
					pUser->SendSystemMessage(L"You have bought 30 Adena Coin.");
				}
			}
		}
		...
		if(augmentation > 0)
		{
			if(pUser->ValidUser())
			{
				pUser->pSD->nAugmentationTmp = augmentation;
			}
		}
	}
	else
...
void CCreature::ChangeTarget(CCreature *pObject, bool ByUser)
{
...
void CCreature::ResetSkillReuse()
{
	...
	for(SkillReuseMap::iterator Iter=this->mSkillReuse.begin();Iter!=this->mSkillReuse.end();Iter++)
	{
		...
		case 443:	//FORCE BARRIER
		//case 1056:	//CANCELLATION
		//case 342:	//s_touch_of_death

		case 22031:	//[s_summon_potions]	skill_id=22031		SKILL QUE DA EL VIP PARA CREAR POTAS, NO REUSE
			{
	...
	if(ValidUser())
	{
		...
		g_SkillReuseManager.ClearAll(GetUser());
	}
}
