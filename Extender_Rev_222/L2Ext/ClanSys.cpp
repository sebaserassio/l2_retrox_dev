...
UINT g_RbPointsRank = 0;
UINT g_ClanWARLevel = 99;

INT32 MaxClanLevel = 8;

BOOL NewAllySystem = FALSE;
...
void ClanSys::Initialize()
{
	...
	g_RbPointsRank = GetPrivateProfileInt(_T("CClanReputationRank"), _T("Enable"), 0, g_ConfigFile);
	/*
	MainClan0Limit = 10
	MainClan1Limit = 15
	MainClan2Limit = 20
	...
	*/
	...
	g_ClanMembersLimit.MainClan5 = GetPrivateProfileInt(section, _T("MainClan5Limit"), 40, g_ConfigFile);
	g_ClanMembersLimit.MainClan6 = GetPrivateProfileInt(section, _T("MainClan6Limit"), 50, g_ConfigFile);

	g_ClanWARLevel = GetPrivateProfileInt(section, _T("ClanWARLevel"), 99, g_ConfigFile);
	...
	MaxClanLevel = GetPrivateProfileInt(section, _T("MaxClanLevel"), 8, g_ConfigFile);


		NewAllySystem = GetPrivateProfileInt(section, _T("NewAllySystem"), 0, g_ConfigFile);
...
void ClanSys::LoadReputationData()
{
	...
	ifstream file(g_ChangeFolders.NpcReputationData_txt.c_str());
	if(file)
	{
		string sLine;
		int nCastle = 0;
		...
		if(Iter!=mNpcReputation.end())
		{
			...
			if(pUser->ValidUser())
			{
				if(CPledge *pPledge = pUser->GetPledge())
				{
					...
					//New Bonus System
					if(pNpc->pSD->nIsEpicNpc == false)
						g_DB.RequestSetPledgeRbPoints(pPledge->pSD->nDBID, 1, Iter->second);
					else
						g_DB.RequestSetPledgeRbPoints(pPledge->pSD->nDBID, 2, Iter->second);
				}
			}
		}
	}
	...
	try
	{
		...
		*/
		...
		for(int n=1;n<5000;n++)
			pPledge->pSD->pPledgeEx->ClansPowerData[n] = 0;
					...
					if(pPledge->pSD->nLevel > 4)
					{
						if( GotEnoughReputationForSubpledge(pPledge, nPledgeType) )
						{
							...
							}else
							{
								...
								{
									if(User *pTarget = g_UserDB.GetUserByName(wMasterName))
									{
										CPledge *pPledge2 = pTarget->GetPledge();
										if(pPledge == pPledge2)
										{
											...
											pUser->GetSocket()->SendSystemMessage(L"Character not clan member.");
									}
									else
										pUser->GetSocket()->SendSystemMessage(L"Character not exist or not online.");
								}else
...
bool ClanSys::Die(User *pUser, CCreature *pEnemy)
{
	if(pEnemy->ValidCreature() && pUser->ValidUser())
	{
		...
		if((pUser->inPvpZone || pUser->pSD->nInCustomPvpZone || pUser->pED->nInFactionPvpZone ) && g_ReputationInPvPZone == FALSE)
			return false;
...
void ClanSys::ApplyNewPenalty()
{
	...
	g_HookManager.WriteMemoryDWORD(0xBF6F44, g_Config.ClanInfo.GetPenalty(CASTLE_ANNOUNCE_TIME));
	g_HookManager.WriteMemoryDWORD(0xBF6F48, g_Config.ClanInfo.GetPenalty(CASTLE_STANDBY_TIME));

	g_HookManager.WriteMemoryDWORD(0xBF6F18, g_Config.ClanInfo.GetPenalty(PLEDGE_RECHALLENGE_PENALTY_TIMEOUT));
	g_HookManager.WriteMemoryDWORD(0xBF6F1C, g_Config.ClanInfo.GetPenalty(PLEDGE_RECHALLENGE_DESPITE_REJECTION_PENALTY_TIMEOUT));
	g_HookManager.WriteMemoryDWORD(0xBF6F3C, g_Config.ClanInfo.GetPenalty(ALLIANCE_RECHALLENGE_PENALTY_TIMEOUT));
	g_HookManager.WriteMemoryDWORD(0xBF6F40, g_Config.ClanInfo.GetPenalty(ALLIANCE_RECHALLENGE_DESPITE_REJECTION_PENALTY_TIMEOUT));
	...
	if(pUser->ValidUser())
	{
		if(pUser->pSD->nIsPledgeLeader)
		{
			if(CPledge *pPledge = pUser->GetPledge())
			{
				...
				//	g_Log.Add(CLog::Error, "[%d]", pPledge->pSD->nLevel);
					if(pPledge->pSD->nLevel> g_ClanWARLevel)
					{
						for(UINT n=1;n<=1000;n++)
						{
							if(CPledge *pPledgeRnd = GetPledgeByID(n))
							{
								if(pPledge != pPledgeRnd)
								{
									if(pPledgeRnd->pSD->nLevel > 5)
									{
										//g_Log.Add(CLog::Error, "22222222222");
								//		wstring clan1(pPledge->pSD->wszClanName);
								//		wstring clan2(pPledgeRnd->pSD->wszClanName);

										wstringstream name1;
										name1 << pPledge->pSD->wszClanName;
										wstring clan1;
										name1 >> clan1;

										std::transform(clan1.begin(), clan1.end(), clan1.begin(), ::tolower);

										wstringstream name2;
										name2 << pPledgeRnd->pSD->wszClanName;
										wstring clan2;
										name2 >> clan2;

										std::transform(clan2.begin(), clan2.end(), clan2.begin(), ::tolower);

										{
											wstring sParams;
											wstringstream wParams2;
											wParams2 << clan1 << " " << clan2;
											getline(wParams2, sParams);

											//bool __stdcall BuilderCmd_declare_clan_war(CUserSocket *pSocket, User *pUser, WCHAR *pBuf)
											typedef void (*f)(CSocket*, User*, WCHAR*);
											f(0x0048D080L)(pUser->GetSocket(), pUser, const_cast< wchar_t* >(sParams.c_str()));
										//	g_Log.Add(CLog::Error, "33333333333");
										}

										{
											wstring sParams;
											wstringstream wParams2;
											wParams2 << clan2 << " " << clan1;
											getline(wParams2, sParams);

											//bool __stdcall BuilderCmd_declare_clan_war(CUserSocket *pSocket, User *pUser, WCHAR *pBuf)
											typedef void (*f)(CSocket*, User*, WCHAR*);
											f(0x0048D080L)(pUser->GetSocket(), pUser, const_cast< wchar_t* >(sParams.c_str()));
										//	g_Log.Add(CLog::Error, "444444444");
										}
									}
								}
							}
...
{
	guard;
	wstringstream file(ReadFileW(g_ChangeFolders.PledgeSkillAcquire_txt));
	wstring line;
	int lineCnt = 0;
	int skillCnt = 0;
	if(file.str().size() == 0)
