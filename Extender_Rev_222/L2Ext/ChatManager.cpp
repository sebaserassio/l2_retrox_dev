...
CChatManager::CChatManager() : m_Enabled(FALSE), m_GlobalChat(FALSE), m_GlobalChatTrade(FALSE), m_BackupInstance(0), m_Reloading(0)
{
...
void CChatManager::LoadINI()
{
	...
	lstrcat(configFile, m_ConfigsIlExt);
	...
	m_GlobalChatTrade = GetPrivateProfileInt(sectionName, L"GlobalChatTrade", 0, configFile);
				...
				case SAY_PARTYROOM_COMMANDER:
					{
						if(m_ChatSetting[type].enabled)
						{
							...
							if(type == SAY_SHOUT && (m_GlobalChat || pUser->pSD->pExData->SkillMod.globalShout) )
							{
							...
							if(type == SAY_TRADE && (m_GlobalChatTrade || pUser->pSD->pExData->SkillMod.globalShout) )
							{
								if(pUser->stopSayTick == 0)
								{
									WCHAR wMessage[4096];
