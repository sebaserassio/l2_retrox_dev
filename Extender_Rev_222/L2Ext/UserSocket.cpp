...
CUserSocket* CUserSocket::OnConstructor(CUserSocket* pSocket, SOCKET s)
{
	...
	if(pNewSocket)
	{
		...
		if(pData)
		{
			...
			pData->shortHWID = 0;
			pData->IsLoadOfflineShop=0;
			pNewSocket->pED = pData;
			pData->FileCorrupt = false;
			pData->TrheadProccesAdrenaline = 0;
			pData->FileCorruptCounter = 0;
			pData->FileCorruptCounterError = 0;
			pData->TrheadProccesAdrenalineCounterError = 0;

//			if((int)g_SocketDB.GetSocketCount() > 150)
//			{
//				pNewSocket = NULL;
//			}
...
void CUserSocket::Close(wstring origen)
{
	if(this)
	{
		typedef void (*f)(CUserSocket*);
		//this->socketStatus = 2;
		g_Log.Add(CLog::Error, "[%s] SocketStatus[%d] account[%S] origen[%S]!", __FUNCTION__, this->socketStatus, this->accountName, origen.c_str());
		f(0x85DEF0L)(this);
	}
}

int CUserSocket::GetPlayTimeTotal()
{
	int valor = 0;
	if(this)
	{
		valor = m_nLeftTime + (time(0) - m_nSocketCharacterTimeStamp);
	}
