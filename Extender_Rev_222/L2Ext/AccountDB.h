...
class CAccountDB : private CThreadSafe
{
	...
	std::map<int, int> CustomSkins;
	std::map<int, int> CustomSex;
public:
	CAccountDB();
	~CAccountDB();
	...
	int GetCustomSkin(const int char_id);
	void AddCustomSkin(int char_id, int skin);
	void AddCustomSkin2(int char_id, int skin);
	int GetCustomSex(const int char_id);
	void AddCustomSex(int char_id, int Sex);
	void AddCustomSex2(int char_id, int Sex);
};
