...
void CHtmlCache::LoadHtmls()
{
	...
	if(m_Enabled)
	{
		...
		lstrcat(path, TEXT("\\"));
		lstrcat(path, g_ChangeFolders.m_Html_cache);
		lstrcat(path, TEXT("*.htm"));
		...
		if(hFind != INVALID_HANDLE_VALUE)
		{
			do
			{
				if((findFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == false)
				{
					...
					filePath << g_ChangeFolders.m_Html_cache << findFileData.cFileName;
					...
					//Bypass the check for the Auction House
					else if( (wcscmp(findFileData.cFileName, L"auction_show_my.htm") == 0) ||
							(wcscmp(findFileData.cFileName, L"auction_show.htm") == 0) ||
							(wcscmp(findFileData.cFileName, L"auction_set_confirm.htm") == 0) ||
							(wcscmp(findFileData.cFileName, L"auction_cancel_confirm.htm") == 0) ||
							(wcscmp(findFileData.cFileName, L"auction_buy_preview.htm") == 0) ||
							(wcscmp(findFileData.cFileName, L"auction_buy_confirm.htm") == 0) )
					{
						wstring name(findFileData.cFileName);
						transform(name.begin(), name.end(), name.begin(), towlower);
						m_Htmls.insert(pair<wstring, wstring>(name, file));
...
const WCHAR* CHtmlCache::Load(wstring name)
{
	guard;
	const WCHAR* wHtml = 0;
	wstring path = g_ChangeFolders.m_Html_cache;
	path += name;
	if(path.find(L".htm") == (path.size() - 4))
	{
		...
		//Bypass the check for the Auction House
		else if( (wcscmp(name.c_str(), L"auction_show_my.htm") == 0) ||
				(wcscmp(name.c_str(), L"auction_show.htm") == 0) ||
				(wcscmp(name.c_str(), L"auction_set_confirm.htm") == 0) ||
				(wcscmp(name.c_str(), L"auction_cancel_confirm.htm") == 0) ||
				(wcscmp(name.c_str(), L"auction_buy_preview.htm") == 0) ||
				(wcscmp(name.c_str(), L"auction_buy_confirm.htm") == 0) )
		{
			transform(name.begin(), name.end(), name.begin(), towlower);
			m_Htmls[name] = file;
			wHtml = m_Htmls[name].c_str();
