...
bool CSkillOperateCondition_op_free_slots::CanUse(CCreature *pCreature, CObject *pTarget)
{
	if(User *pUser = pCreature->SafeCastUser())
	{	
		//m_FreeSlots

		int Total_Slots = pUser->pED->inventory_limit;
		int Used_Slots = pUser->CountPlayerInventoryItems();

		if( (Total_Slots-Used_Slots) >= m_FreeSlots)
		{
			g_Log.Add(CLog::Blue, "Op Condition Check - Free Slots. totalslots[%d] usedslots[%d] tocheck[%d]", Total_Slots, Used_Slots, m_FreeSlots);
			return true;
		}
	}
	return false;
...
bool CSkillOperateCondition_op_sweep_spoil::CanUse(CCreature *pCreature, CObject *pTarget)
{
	if(User *pUser = pCreature->SafeCastUser())
	{	
...
bool CSkillOperateCondition_op_remain_hp_diff::CanUse(CCreature *pCreature, CObject *pTarget)	//lo agrege para setear el skill revival con menos de 1000 de hp
{
	if(pCreature->ValidCreature())
	{
		double RemainHpDiff = pCreature->pSD->fHP;
		if(RemainHpDiff <= this->nDiff)
			return true;
		else
			return false;
	}
