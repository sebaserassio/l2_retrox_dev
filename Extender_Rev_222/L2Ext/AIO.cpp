...
{
	m_data.clear();
	wstringstream file(ReadFileW(g_ChangeFolders.AIOData_txt));
	if(file)
	{
		wstring line;
		while(getline(file, line))
		{
			...
			if(line.find(L"setting_begin") == 0)
			{
				...
				wstring items = Parser::ParseOptionString(line, L"reward_id");
				items = Parser::Replace(items, ';', ' ');
				items = Parser::Replace(items, '{', ' ');
				items = Parser::Replace(items, '}', ' ');
				wstringstream respawnStream(items);
				int item_id = 0;
				while(respawnStream >> item_id)
				{
					data.rewardId.push_back(item_id);
				}
...
void CAIOSystem::BecomeAIO(User *pUser, UINT id, bool byGM)
{
	...
	if(m_enabled)
	{
		if(pUser->ValidUser())
		{
			if(pUser->pED->aioId == 0)
			{
				if(pUser->pSD->nSubClassIndex == 0 && pUser->pSD->nSubJobClass[1]<1)
				{
					map<UINT, AIOData>::iterator it = m_data.find(id);
					if(it != m_data.end())
					{
						...
						if(aio.enabled)
						{
							...
							if(aio.minLevel <= pUser->pSD->nLevel || byGM)
							{
								if(aio.priceId > 0 && aio.priceAmount > 0 && !byGM)
								{
									if(!pUser->DeleteItemInInventory(aio.priceId, aio.priceAmount))
									{
										...
										msg1.AddText(L"No tienes el Item necesario para convertirte en AIO-Buffer.");
								...
								for(list<int>::iterator Iter = aio.rewardId.begin(); Iter!=aio.rewardId.end();Iter++)
								{
									int ItemId = (*Iter);
									pUser->AddItemToInventory(ItemId, 1);
								}
								...
								// Convertir wszName a std::wstring
								std::wstring originalName(pUser->pSD->wszName);

								// Definir los prefijos y sufijos que queremos añadir
								std::wstring prefix = L"*Aio-";
								std::wstring suffix = L"*";

								// Concatenar las cadenas
								std::wstring newName = prefix + originalName + suffix;
							...
							{
								CSystemMessage msg1(SystemMessage::S1_POPUP);
								msg1.AddText(L"ENo tienes el level necesario para convertirte en AIO-Buffer.");
								pUser->pSocket->SendSystemMessage(&msg1);
						...
						{
							CSystemMessage msg1(SystemMessage::S1_POPUP);
							msg1.AddText(L"El servicio de AIO-Buffer esta deshabilitado.");
							pUser->pSocket->SendSystemMessage(&msg1);
				...
				{
					CSystemMessage msg1(SystemMessage::S1_POPUP);
					msg1.AddText(L"Debes estar en Main-Class para convertirte en  AIO-Buffer.");
					pUser->pSocket->SendSystemMessage(&msg1);
			...
			{
				CSystemMessage msg1(SystemMessage::S1_POPUP);
				msg1.AddText(L"Ya eres un AIO-Buffer.");
				pUser->pSocket->SendSystemMessage(&msg1);
...
void CAIOSystem::TimerExpired(User *pUser)
{
	...
	if(m_enabled)
	{
		if(pUser->ValidUser())
		{
			if(pUser->pED->aioId && pUser->pED->aioTimestamp != -1)
			{
				...
				if(pUser->pED->aioTimestamp < currentTime)
				{
					...
					if(it!=m_data.end())
					{
						...
						for(list<int>::iterator Iter = it->second.rewardId.begin(); Iter!=it->second.rewardId.end();Iter++)
						{
							int ItemId = (*Iter);
							if(pUser->inventory.HaveItemByClassID(ItemId, 1))
							{
								pUser->DeleteItemInInventory(ItemId, 1);
							}
						}
					...
					// Convertir wszName a std::wstring
					std::wstring fullName(pUser->pSD->wszName);

					// Definir los prefijos y sufijos que queremos eliminar
					std::wstring prefix = L"*Aio-";
					std::wstring suffix = L"*";

					// Verificar y eliminar el prefijo
					if (fullName.find(prefix) == 0) {
						fullName = fullName.substr(prefix.length());
					}

					// Verificar y eliminar el sufijo
					if (fullName.rfind(suffix) == fullName.length() - suffix.length()) {
						fullName = fullName.substr(0, fullName.length() - suffix.length());
					}

					//pUser->Kick();
					NuevasFunciones::setCharacterName(pUser->nDBID, fullName);


					pUser->lpSharedDataLock->Lock(__FILEW__, __LINE__);
					wcsncpy_s(pUser->pSD->wszName, sizeof(pUser->pSD->wszName) / sizeof(wchar_t), fullName.c_str(), _TRUNCATE);
					pUser->pSD->wszName[24] = L'\0';
					pUser->lpSharedDataLock->Unlock();
