...
void CHeroSystem::LoadINI()
{
	...
	m_SkillsInAllSubClass = GetPrivateProfileInt(section, _T("SkillsInAllSubClass"), FALSE, g_ConfigFile);

	TCHAR temp[8190];
	if(GetPrivateProfileString(section, _T("AcquireSkillList"), 0, temp, 0x8000, g_ConfigFile))
	{
	...
	if(ret)
	{
		...
		if(pUser->ValidUser())
		{
			if(pAtomic->heroType == 2)
			{
				...
				if(pUser->pSD->nSubClassIndex == 0 || g_HeroSystem.m_SkillsInAllSubClass)
				{
				...
				g_Achivments.SetUser_Hero(pUser);
			}
		}
	}
...
void CHeroSystem::TimerExpired(User *pUser)
{
	...
	if(m_enabled)
	{
		if((pUser->pSD->nSubClassIndex == 0 || m_SkillsInAllSubClass) && pUser->pSD->nHero == 2)
		{
		...
		}else if(pUser->pSD->nHero != 2 || (pUser->pSD->nSubClassIndex != 0 && !m_SkillsInAllSubClass))
		{
			if(m_skills.size() > 0)
			{
				bool done = false;
