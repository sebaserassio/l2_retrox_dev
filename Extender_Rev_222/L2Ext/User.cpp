...
extern bool g_verificarSiTienenDestroOPala;
extern int g_pasiva_mago_no_pala_destro;
extern int g_pasiva_mago_no_pala;
extern int g_pasiva_mago_no_destro;

extern int g_pasiva_warrior_no_pala_destro;
extern int g_pasiva_warrior_no_pala;
extern int g_pasiva_warrior_no_destro;
...
void User::OnCreate(User *pUser)
{
	...
	if(pUser)
	{
		...
		if(pUser->pED)
		{
			...
			pUser->pED->nCustomSubClassIndex = 0;
			pUser->pED->nCustomSubClass[0] = -1;
			pUser->pED->nCustomSubClass[1] = -1;
			pUser->pED->nCustomSubClass[2] = -1;
			pUser->pED->nCustomSubClass[3] = -1;

			pUser->pED->nInCustomHeroTime = 0;
...
void User::SendCleanFLUSH()
{
	pSocket->Send("cdd", 0x87, 21, 1);
}

void User::SendCleanGARBAGE()
{
	pSocket->Send("cdd", 0x87, 21, 2);
}
...
void User::SendGMItemList(User* pUser)
{
	typedef void(*sil)(User*, BYTE, WORD, CUserSocket*);
	sil _sil = (sil) 0x00819C40;
	_sil(this, 0, 1, pUser->pSocket);
}
...
bool User::UseItem(CItem *pItem, int nForce)
{
	...
	if(pItem)
	{
		bool ret = _UseItem(this, pItem, nForce);
		//g_Log.Add(CLog::Error, "[%s] 1111111111111!",__FUNCTION__);
		if(pItem->nAugmentationID)
		{
			//g_Log.Add(CLog::Error, "[%s] pItem->nAugmentationID! [%d]",__FUNCTION__, pItem->nAugmentationID);
			ValidateSkillList();
...
bool User::UnEquipItem(CItem* pItem)
{
	typedef bool(__fastcall *_EquipI)(User*, CItem*);
	_EquipI EquipI = (_EquipI) 0x00826A90;
	return EquipI(this, pItem);
}
...
{
	typedef bool(*f)(User*, WCHAR *);
	return f(0x821D80L)(this, pwCharName);
}
...
void User::AutoSkills()
{
	...
	if(g_MiAutoSkill)
	{
		try
		{
			...
			//g_Log.Add(CLog::Error, "[%s] 11111111 nClassID[%d] nLevel[%d]", __FUNCTION__, nClassID, nLevel);
			...
			if (nLevel < g_MiAutoSkillMaxLevel+1)
			{
				...
				for(map<int, int>::iterator it = mAcquirable.begin(); it!=mAcquirable.end();it++)
				{
					...
					//g_Log.Add(CLog::Error, "[%s] 22222222 skillId[%d] skillLevel[%d]", __FUNCTION__, skillId, skillLevel);
					...
					{
						//g_Log.Add(CLog::Error, "[%s] 33333333 Iter2->first[%d] skillId[%d]", __FUNCTION__, Iter2->first, skillId);
						if(Iter2->first == skillId)
						{
							if (skillLevel <= Iter2->second)
							{
								skillLevel =0;
						//		g_Log.Add(CLog::Error, "[%s] 444444444 Iter2->first[%d] skillId[%d] skillLevel[%d] Iter2->second[%d]", __FUNCTION__, Iter2->first, skillId, skillLevel, Iter2->second);
							}
						}
					}
					//-------------------------------------------------------------------------------.
		...
		{
			g_Log.Add(CLog::Error, "[%s] Exception: %s", __FUNCTION__, e.what());
		}
		catch(...)
		{
			g_Log.Add(CLog::Error, "[%s] Unknown exception in AutoSkills()", __FUNCTION__);
		}
...
void User::VerificarSiTienenDestroOPala()
{
	guard;

	if(g_verificarSiTienenDestroOPala)
	{

		if (pSD->nSubJobClass[0] >= 0 && pSD->nSubJobClass[1] >= 0 && pSD->nSubJobClass[2] >= 0 && pSD->nSubJobClass[3] >= 0)
		{

			if(pED->EsMago)
			{
				bool TienePala = true;
				bool TieneDestro = true;
				//PALA
				if (pSD->nSubJobClass[0] != 5 && pSD->nSubJobClass[1] != 5 && pSD->nSubJobClass[2] != 5 && pSD->nSubJobClass[3] != 5 && pSD->nSubJobClass[0] != 90 && pSD->nSubJobClass[1] != 90 && pSD->nSubJobClass[2] != 90 && pSD->nSubJobClass[3] != 90)
				{
					TienePala = false;
				}

				//DESTRO
				if (pSD->nSubJobClass[0] != 46 && pSD->nSubJobClass[1] != 46 && pSD->nSubJobClass[2] != 46 && pSD->nSubJobClass[3] != 46 && pSD->nSubJobClass[0] != 113 && pSD->nSubJobClass[1] != 113 && pSD->nSubJobClass[2] != 113 && pSD->nSubJobClass[3] != 113)
				{
					TieneDestro = false;
				}

				if(!TienePala && !TieneDestro)
				{
					if(g_pasiva_mago_no_pala_destro)
					{
						AcquireSkill(g_pasiva_mago_no_pala_destro, 1);
						ValidateParameters();
					}
				}
				else if(!TienePala)
				{
					if(g_pasiva_mago_no_pala)
					{
						AcquireSkill(g_pasiva_mago_no_pala, 1);
						ValidateParameters();
					}
				}
				else if(!TieneDestro)
				{
					if(g_pasiva_mago_no_destro)
					{
						AcquireSkill(g_pasiva_mago_no_destro, 1);
						ValidateParameters();
					}
				}
			...
			else
			{
				...
				if(!TienePala && !TieneDestro)
				{
					if(g_pasiva_warrior_no_pala_destro)
					{
						AcquireSkill(g_pasiva_warrior_no_pala_destro, 1);
						ValidateParameters();
					}
				}
				else if(!TienePala)
				{
					if(g_pasiva_warrior_no_pala)
					{
						AcquireSkill(g_pasiva_warrior_no_pala, 1);
						ValidateParameters();
					}
				}
				else if(!TieneDestro)
				{
					if(g_pasiva_warrior_no_destro)
					{
						AcquireSkill(g_pasiva_warrior_no_destro, 1);
						ValidateParameters();
					}
				}
			}
...
void User::Kick()
{
	...
	{
		StopMove();
		pSocket->Close(L"User::Kick");
		g_Log.Add(CLog::Error, "[%s] [%S]", __FUNCTION__, pSD->wszName);
	}
}
...
//				SendSystemMessage(&msg);
				...
				wstring m_Message = L"";
				m_Message = L"$s1 disappears $s2 dia(s) $s3 hora(s) $s4 min(s).";
...
void User::SendLifeTimeMessageForItem(CItem* pItem)
{
	if (!pItem || pItem->nLifeTime <= 0)
		return;

	time_t currentTime = time(0);
	INT32 diff = pItem->nLifeTime - currentTime;
	...
	if (diff > 60)
	{
		...
		wstring m_Message = L"$s1 desaparece en $s2 dia(s), $s3 hora(s), $s4 min(s).";

		size_t pos = m_Message.find(L"$s1");
		while (pos != wstring::npos)
		{
			m_Message.replace(pos, 3, g_ItemDBEx.GetItemName(pItem->pSID->nItemID).c_str());
			pos = m_Message.find(L"$s1");
		}
	...
	{
		wstring m_Message = L"$s1 desaparecerá en menos de un minuto.";
...
void User::ValidateTerritory()
{
	...
	if(ValidUser())
	{
		...
		if(td.territoryId[NO_NOBLESS] != 0)
		{
			if(pSD->nNobless != 0)
			{
				if(CNoNobleZoneTerritory *pTerritory = g_TerritoryChecker.GetTerritory(td.territoryId[NO_NOBLESS])->SafeCastNoNobleZone())
				{
					if(!pSD->nBuilder)
					{
						CSystemMessage msg(2907);
						msg.AddText(L"No puedes ingresar a esta Zona porque ya eres noble.");
						SendSystemMessage(&msg);
		...
		if(td.territoryId[HORARIO_ZONE] != 0)
		{
			if(CHorarioZoneTerritory *pTerritory = g_TerritoryChecker.GetTerritory(td.territoryId[HORARIO_ZONE])->SafeCastHorarioZone())
			{
				if (!pTerritory->IsAllowedNow()) {

					if(!pSD->nBuilder)
					{
						wchar_t txt[128];
						wsprintfW(txt, L"La zona esta abierta de %02d:%02d a %02d:%02d.",
							pTerritory->GetHourInit(),  pTerritory->GetMinuteInit(),
							pTerritory->GetHourEnd(),   pTerritory->GetMinuteEnd());
		...
		//NickNameChange Zone
		if(pED->inZonePvPRank)
		{
			if(td.territoryId[PVP_RANK_ZONE] == 0)
			{
				pED->inZonePvPRank = NULL;
				SetUserInfoChanged();
				SetCharInfoChanged();
			}
		}else
		{
			if(td.territoryId[PVP_RANK_ZONE])
			{
				pED->inZonePvPRank = td.territoryId[PVP_RANK_ZONE];
				SetUserInfoChanged();
				SetCharInfoChanged();
			}
		}

		if(pED->NoDownExpZone)
		{
			if(td.territoryId[NO_DOWN_EXP_ZONE] == 0)
			{
				pED->NoDownExpZone = false;
			}
		}else
		{
			if(td.territoryId[NO_DOWN_EXP_ZONE])
			{
				pED->NoDownExpZone = true;
			}
		}
		...
		//inZoneNoVisualEffect Zone
		if(pED->inZoneNoVisualEffect)
		{
			if(td.territoryId[NO_VISUAL_EFFECT_ZONE] == 0)
			{
				pED->inZoneNoVisualEffect = NULL;
				SetUserInfoChanged();
				SetCharInfoChanged();
			}
		}else
		{
			if(td.territoryId[NO_VISUAL_EFFECT_ZONE])
			{
				pED->inZoneNoVisualEffect = td.territoryId[NO_VISUAL_EFFECT_ZONE];
				SetUserInfoChanged();
				SetCharInfoChanged();
			}
		}
		...
		//ItemDrop Zone
		if(pED->DropItemTerritoryIN)
		{
			if(td.territoryId[ITEMDROP_ZONE] == 0)
			{
				pED->DropItemTerritoryIN = false;
				pED->DropItemTerritory_ItemId = 0;
				pED->DropItemTerritory_ItemMin = 0;
				pED->DropItemTerritory_ItemMax = 0;
				pED->DropItemTerritory_ItemChance = 0;
			}
		}else
		{
			if(td.territoryId[ITEMDROP_ZONE] != 0)
			{
				if(CItemDropTerritory *pT = g_TerritoryChecker.GetTerritory(td.territoryId[ITEMDROP_ZONE])->SafeCastItemDrop())
				{
					pED->DropItemTerritoryIN = true;
					pED->DropItemTerritory_ItemId = pT->GetItemId();
					pED->DropItemTerritory_ItemMin = pT->GetItemMin();
					pED->DropItemTerritory_ItemMax = pT->GetItemMax();
					pED->DropItemTerritory_ItemChance = pT->GetItemChance();

					for(int n=0;n<11;n++)
					{
						pED->DropItemTerritory_ItemArmorSet[n] = pT->ItemArmorSet[n];
						pED->DropItemTerritory_ArmorRate[n] = pT->ArmorRate[n];
					}
			...
			if(pSD->nActiveUser)
			{
				if(!pSD->nBuilder)
				{
					if(CPartyTerritory *pTerritory = g_TerritoryChecker.GetTerritory(td.territoryId[PARTY_ZONE])->SafeCastParty())
					{
						...
						{
							UserVector dentro;
							members = pParty->GetAllMembersInZone(dentro, pTerritory->GetMinPos(), pTerritory->GetMaxPos(), true);

						}
						if(!pTerritory->CanEnter(members))
						{
							pSD->pExData->m_TickParty++;

							if(pSD->pExData->m_TickParty > 10)
								pSocket->SendSystemMessage(L"Si no cumplen las condiciones seran expulsados en unos segundos.");

							if(pSD->pExData->m_TickParty > 48)
							{
								IVector ipos(pTerritory->GetKickOutPos());
								SendSystemMessage(L"No cumples la condicion para estar aqui!");

								CSystemMessage msg(SystemMessage::S1_POPUP);
								msg.AddText(L"No cumples la condicion para estar aqui!");
								SendSystemMessage(&msg);
...
void User::HideAbnormal(AbnormalData& abnormal, UINT expireTime)
{
	...
	if(pED->hiddenAbnormals.size() < 60)
	{
...
bool User::ValidateStorageSlot(User *pUser)
{
	...
	if(pUser->ValidUser())
	{
		if ( pUser->pSD->nRace == 4 ) //Dwarf Race
		{
			...
			{
				if ( Sell_Slots > 12 )
					pUser->pSD->m_nPrivateStoreLimit = 12;
				else
					pUser->pSD->m_nPrivateStoreLimit = Sell_Slots;
			}
			else
			  pUser->pSD->m_nPrivateStoreLimit = 0;
			...
			{
			  if ( Buy_Slots > 12 )
				pUser->pSD->m_nPrivateStoreBuyLimit = 12;
			  else
				pUser->pSD->m_nPrivateStoreBuyLimit = Buy_Slots;
			}
			else
			  pUser->pSD->m_nPrivateStoreBuyLimit = 0;
		...
		else // All Other Races
		{
			...
			{
				if ( Sell_Slots > 11 )
					pUser->pSD->m_nPrivateStoreLimit = 11;
				else
					pUser->pSD->m_nPrivateStoreLimit = Sell_Slots;
			}
			else
			...
			{
			  if ( Buy_Slots > 11 )
				pUser->pSD->m_nPrivateStoreBuyLimit = 11;
			  else
				pUser->pSD->m_nPrivateStoreBuyLimit = Buy_Slots;
			}
			else
		...
		Packet.WriteD(pUser->pSD->m_nPrivateStoreLimit); //private sell
		Packet.WriteD(pUser->pSD->m_nPrivateStoreBuyLimit); //private buy
