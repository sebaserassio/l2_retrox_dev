...
PenaltyChancePK=66
;Not Use Death Penaly / And Not drop items
GoldType=1
*/
...
//	m_PenaltySkillId = GetPrivateProfileInt(section, _T("PenaltySkillId"), 0, g_ConfigFile);
	...
	TCHAR temp[8190];
	if(GetPrivateProfileString(section, _T("PenaltySkillId"), 0, temp, 0x8000, g_ConfigFile))
	{
		wstringstream skillStream(temp);
		wstring skillName;
		skillStream >> skillName;
		INT32 skillId = 0, level = 0;
		g_SkillDB.GetSkillIdLevelByName((PWCHAR)skillName.c_str(), &skillId, &level);
		if(skillId > 0)
			m_PenaltySkillId = skillId;
		else
			m_PenaltySkillId = 0;
	}
	...
	m_PenaltyGoldType = GetPrivateProfileInt(section, _T("GoldType"), 0, g_ConfigFile);
}
