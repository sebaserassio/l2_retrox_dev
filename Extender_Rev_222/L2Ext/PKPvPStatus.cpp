...
void CPKPvPStatus::Init()
{
	...
	if(m_Enabled)
	{
		...
		g_DB.RequestLoadDailyStatus();
		g_DB.RequestLoadSemanaStatus();
	}
}
...
void CPKPvPStatus::RequestDailyStatus(User *pUser, UINT page)
{
	guard;
	if(m_Enabled)
	{
		wstring html = L"<html><head></head><body><br><br><br><center>Loading data...<br>Please try again later...</center></body></html>";
		m_Lock.Enter();
		if(m_PagesDaily.size() > 0 && page < m_PagesDaily.size())
		{
			html = m_PagesDaily[page];
		}
		m_Lock.Leave();

		PlayerAction::ShowHTML(pUser, L"daily_status.htm", html.c_str(), 0);
	}
	unguard;
}


void CPKPvPStatus::RequestSemanaStatus(User *pUser, UINT page)
{
	guard;
	if(m_Enabled)
	{
		wstring html = L"<html><head></head><body><br><br><br><center>Loading data...<br>Please try again later...</center></body></html>";
		m_Lock.Enter();
		if(m_PagesSemana.size() > 0 && page < m_PagesSemana.size())
		{
			html = m_PagesSemana[page];
		}
		m_Lock.Leave();
...
void CPKPvPStatus::OnDBLoadPK(UINT charId, wstring name, UINT classId, INT32 level, INT32 pvp, INT32 pk, wstring nameClan, wstring nameAlly, UINT crestClan, UINT crestAlly)
{
	guard;
	...
	pui.nameClan = nameClan;
	pui.nameAlly = nameAlly;
	pui.crestClan = crestClan;
	pui.crestAlly = crestAlly;
...
void CPKPvPStatus::OnDBLoadPvP(UINT charId, wstring name, UINT classId, INT32 level, INT32 pvp, INT32 pk, wstring nameClan, wstring nameAlly, UINT crestClan, UINT crestAlly)
{
	guard;
...
void CPKPvPStatus::OnDBLoadDaily(UINT charId, wstring name, UINT classId, INT32 level, INT32 pvp, INT32 pk, wstring nameClan, wstring nameAlly, UINT crestClan, UINT crestAlly)
{
	guard;
	...
	bool done = false;
	m_Lock.Enter();
	for(vector<PKPvPUserInfo>::iterator it = m_DBDaily.begin(); it != m_DBDaily.end();it++)
	{
		if(it->pvpAmount < pvp)
		{
			m_DBDaily.insert(it, pui);
			done = true;
			break;
		}
	}
	if(!done)
	{
		m_DBDaily.push_back(pui);
	}
	m_Lock.Leave();
	unguard;
}

void CPKPvPStatus::OnDBLoadSemana(UINT charId, wstring name, UINT classId, INT32 level, INT32 pvp, INT32 pk, wstring nameClan, wstring nameAlly, UINT crestClan, UINT crestAlly)
{
	guard;
	...
	bool done = false;
	m_Lock.Enter();
	for(vector<PKPvPUserInfo>::iterator it = m_DBSemana.begin(); it != m_DBSemana.end();it++)
	{
		if(it->pvpAmount < pvp)
		{
			m_DBSemana.insert(it, pui);
			done = true;
			break;
		}
	}
...
void CPKPvPStatus::SendHeroListPvP(User *pUser, INT32 desde, INT32 cantidad)
{
	if(m_DBPvP.size() > 0)
	{
		if(pUser->ValidUser() && pUser->pSD)
		{
			CPacket packet;

			INT32 nCounts = 0;
			INT32 contarContinues = 0;
			try
			{
				m_Lock.Enter();
				...
				{
					contarContinues++;
					if(contarContinues < desde)
						continue;

					PKPvPUserInfo& pui = m_DBPvP[n];

					packet.WriteS(pui.name.c_str());
					packet.WriteD(pui.classId);
					packet.WriteS(pui.nameClan.c_str());
					packet.WriteD(pui.crestClan);
					packet.WriteS(pui.nameAlly.c_str());
					packet.WriteD(pui.crestAlly);
					packet.WriteD(pui.pvpAmount);
					nCounts++;
				...
				pUser->pSocket->Send("chdb", 0xFE, 0x23, nCounts, packet.GetSize(), packet.GetBuff());
...
void CPKPvPStatus::SendHeroListPK(User *pUser, INT32 desde, INT32 cantidad)
{
	if(m_DBPK.size() > 0)
	{
		if(pUser->ValidUser() && pUser->pSD)
...
void CPKPvPStatus::SendHeroListDaily(User *pUser, INT32 desde, INT32 cantidad)
{
	if(m_DBDaily.size() > 0)
	{
		if(pUser->ValidUser() && pUser->pSD)
		{
			...
			{
				m_Lock.Enter();
				for(UINT n=0;n<m_DBDaily.size();n++)
				{
					contarContinues++;
					if(contarContinues < desde)
						continue;
...
void CPKPvPStatus::SendHeroListSemana(User *pUser, INT32 desde, INT32 cantidad)
{
	if(m_DBSemana.size() > 0)
	{
		if(pUser->ValidUser() && pUser->pSD)
		{
			CPacket packet;
			...
			{
				m_Lock.Enter();
				for(UINT n=0;n<m_DBSemana.size();n++)
				{
					contarContinues++;
					if(contarContinues < desde)
						continue;
					...
					PKPvPUserInfo& pui = m_DBSemana[n];
...
void CPKPvPStatus::TimerExpired()
{
	...
	if(m_Enabled)
	{
		...
		{
			UINT pagesPK = 0, pagesPvP = 0, pagesDaily = 0, pagesSemana = 0;
			m_Lock.Enter();
			//Generate pages
			GeneratePagesPK();
			GeneratePagesPvP();
			...
			pagesDaily = static_cast<UINT>(m_PagesDaily.size());
			pagesSemana = static_cast<UINT>(m_PagesSemana.size());
...
void CPKPvPStatus::GeneratePagesDaily()
{
	guard;
	const WCHAR* wHTML = g_HtmlCache.Get(L"daily_status.htm");
	if(wHTML)
	{
		if(m_DBDaily.size() > 0)
		{
			...
			m_PagesDaily.clear();
			UINT page = 0;
			UINT totalPages = static_cast<UINT>(m_DBDaily.size() / 15);
			if((m_DBDaily.size() % 15) != 0)
			{
				totalPages++;
			}
			WCHAR temp[8190] = { 0 };
...
void CPKPvPStatus::GeneratePagesSemana()
{
	guard;
	const WCHAR* wHTML = g_HtmlCache.Get(L"semana_status.htm");
	if(wHTML)
	{
		if(m_DBSemana.size() > 0)
		{
			...
			m_PagesSemana.clear();
			UINT page = 0;
			UINT totalPages = static_cast<UINT>(m_DBSemana.size() / 15);
			if((m_DBSemana.size() % 15) != 0)
			{
				totalPages++;
			}
			WCHAR temp[8190] = { 0 };
