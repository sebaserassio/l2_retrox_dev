...
extern double g_DamagePlayerMultiplerWarriorAMago_none;
extern double g_DamagePlayerMultiplerWarriorAMago_sword;
extern double g_DamagePlayerMultiplerWarriorAMago_blunt;
extern double g_DamagePlayerMultiplerWarriorAMago_dagger;
extern double g_DamagePlayerMultiplerWarriorAMago_pole;
extern double g_DamagePlayerMultiplerWarriorAMago_fist;
extern double g_DamagePlayerMultiplerWarriorAMago_bow;
extern double g_DamagePlayerMultiplerWarriorAMago_etc;
extern double g_DamagePlayerMultiplerWarriorAMago_dual;
extern double g_DamagePlayerMultiplerWarriorAMago_dualfist;
extern double g_DamagePlayerMultiplerWarriorAMago_fishing_rod;

extern double g_DamagePlayerMultiplerWarriorAMago_noneTwo;
extern double g_DamagePlayerMultiplerWarriorAMago_swordTwo;
extern double g_DamagePlayerMultiplerWarriorAMago_bluntTwo;
extern double g_DamagePlayerMultiplerWarriorAMago_daggerTwo;
extern double g_DamagePlayerMultiplerWarriorAMago_poleTwo;
extern double g_DamagePlayerMultiplerWarriorAMago_fistTwo;
extern double g_DamagePlayerMultiplerWarriorAMago_bowTwo;
extern double g_DamagePlayerMultiplerWarriorAMago_etcTwo;
extern double g_DamagePlayerMultiplerWarriorAMago_dualTwo;
extern double g_DamagePlayerMultiplerWarriorAMago_dualfistTwo;
extern double g_DamagePlayerMultiplerWarriorAMago_fishing_rodTwo;

extern double g_DamagePlayerMultiplerWarriorAWarrior_none;
extern double g_DamagePlayerMultiplerWarriorAWarrior_sword;
extern double g_DamagePlayerMultiplerWarriorAWarrior_blunt;
extern double g_DamagePlayerMultiplerWarriorAWarrior_dagger;
extern double g_DamagePlayerMultiplerWarriorAWarrior_pole;
extern double g_DamagePlayerMultiplerWarriorAWarrior_fist;
extern double g_DamagePlayerMultiplerWarriorAWarrior_bow;
extern double g_DamagePlayerMultiplerWarriorAWarrior_etc;
extern double g_DamagePlayerMultiplerWarriorAWarrior_dual;
extern double g_DamagePlayerMultiplerWarriorAWarrior_dualfist;
extern double g_DamagePlayerMultiplerWarriorAWarrior_fishing_rod;

extern double g_DamagePlayerMultiplerWarriorAWarrior_noneTwo;
extern double g_DamagePlayerMultiplerWarriorAWarrior_swordTwo;
extern double g_DamagePlayerMultiplerWarriorAWarrior_bluntTwo;
extern double g_DamagePlayerMultiplerWarriorAWarrior_daggerTwo;
extern double g_DamagePlayerMultiplerWarriorAWarrior_poleTwo;
extern double g_DamagePlayerMultiplerWarriorAWarrior_fistTwo;
extern double g_DamagePlayerMultiplerWarriorAWarrior_bowTwo;
extern double g_DamagePlayerMultiplerWarriorAWarrior_etcTwo;
extern double g_DamagePlayerMultiplerWarriorAWarrior_dualTwo;
extern double g_DamagePlayerMultiplerWarriorAWarrior_dualfistTwo;
extern double g_DamagePlayerMultiplerWarriorAWarrior_fishing_rodTwo;





extern double g_DamageWarriorsAWarriorMinimo;
extern double g_DamageWarriorsAWarriorMaximo;

extern double g_DamageWarriorsAMagoMinimo;
extern double g_DamageWarriorsAMagoMaximo;

extern double g_MagoVsMago;
extern double g_MagoVsWarrior;
extern double g_WarriorVsMago;
extern double g_WarriorVsWarrior;
		...
		try
		{
			if(pCreature->ValidCreature())
			{
				if(pCreature->ValidUser() || pCreature->IsSummon() || pCreature->IsPet())
				{
					...
					{
						//Custom Spawns Zone
						if (td.territoryId[TerritoryCustomSpawnsZone] != 0)
						{
							if (pUser->pED->inCustomSpawnZone == FALSE)
							{
								pUser->pED->inCustomSpawnZone = TRUE;

								UINT territoryId = td.territoryId[TerritoryCustomSpawnsZone];
								if(CTerritoryCustomSpawnsZone* pTerritory = g_TerritoryChecker.GetTerritory(td.territoryId[TerritoryCustomSpawnsZone])->SafeCastTerritoryCustomSpawnsZone())
								{
									std::memcpy(pUser->pED->m_CustomSpawnPoints, pTerritory->m_SpawnPoints, sizeof(pTerritory->m_SpawnPoints));
									pUser->pED->m_CustomSpawnCount = pTerritory->m_SpawnCount;
									pUser->pED->m_CustomSpawnRet = pTerritory->GetOutPos();
						...
						else
						{
							...
							{
								pUser->pED->inCustomSpawnZone = FALSE;
								pUser->pED->m_CustomSpawnCount = 0;
							}
						}
					}
						...
						if(pCreature->pSD->nInCustomPvpZone > 0)
						{
							if(td.territoryId[PVP_ZONE] == 0)
							{
								...
								if(User *pUser = pCreature->GetUser())
								{
									...
									CSystemMessage msg(2914);
									msg.AddText(L"You have left a PvP Zone.");
									pUser->SendSystemMessage(&msg);
						...
						}else
						{
							if(td.territoryId[PVP_ZONE] != 0)
							{
								...
								if(User *pUser = pCreature->GetUser())
								{
									...
									CSystemMessage msg(2914);
									msg.AddText(L"You have entered a PvP Zone.");
									pUser->SendSystemMessage(&msg);
								}
							}
						}
					...
					if(User *pUser = pCreature->GetUser())
					{
						if(pUser->pED->inZoneCustomBuffBackup)
						{
							if(td.territoryId[SKILL_BACKUP_ZONE] == 0)
							{
								pUser->pED->inZoneCustomBuffBackup = 0;
							}
						}
						else
						{
							if(td.territoryId[SKILL_BACKUP_ZONE] != 0)
							{
								pUser->pED->inZoneCustomBuffBackup = td.territoryId[SKILL_BACKUP_ZONE];
							}
						}
					}
					...
					//if(td.territoryId[PVP_ZONE] == 0)
					//{
						if(pCreature->GetUser() && pCreature->GetUser()->pED->nInPvpZoneSteal == true )
						{
							if(td.territoryId[PVP_ZONE_STEAL] == 0)
							{
								if(User *pUser = pCreature->GetUser())
...
//									pUser->SetCharInfoChanged();
									...
									CSystemMessage msg(2920);
									msg.AddText(L"You have left a Steal Zone.");
									pUser->SendSystemMessage(&msg);
...
//									pUser->SetCharInfoChanged();
									...
									CSystemMessage msg(2920);
									msg.AddText(L"You have entered a Steal Zone.");
									pUser->SendSystemMessage(&msg);
				...
				if(CAntiExploit::ValidateStats(pCreature))
				{
					...
					if(pCreature->ValidUser())
					{
						...
						if(CAntiExploit::ValidateStats(pCreature))
						{
							...
							if(pCreature->ValidUser())
							{
								pCreature->GetUser()->GetSocket()->Close(L"CAntiExploit::ValidateStats");
							}
						}
					}
				}
...
{
	//g_Log.Add(CLog::Error, "[%s] 1111111", __FUNCTION__);
...
void CreatureAction::OnSetSkillUsableTime2(CCreature *pCreature, int nSkillID, int nLevel, DWORD *stamp, DWORD Reuse)
{
	...
	if(pInfo)
	{
		//g_Log.Add(CLog::Error, "[%s] 22222222", __FUNCTION__);
		if(pInfo->staticReuseTime > 0)
		{
			//g_Log.Add(CLog::Error, "[%s] 3333333333", __FUNCTION__);
		//	g_Log.Add(CLog::Blue, "[%s] Reuse[%d] Static[%d]", __FUNCTION__, Reuse, pInfo->staticReuseTime);
			Reuse = pInfo->staticReuseTime;
		}
		else
		{
			//g_Log.Add(CLog::Error, "[%s] 44444444", __FUNCTION__);
			if(pInfo->staticReuseTimeWarrior > 0 && pInfo->staticReuseTimeMago > 0)
			{
				//g_Log.Add(CLog::Error, "[%s] 55555555", __FUNCTION__);
				if(User *pUser = pCreature->SafeCastUser())
				{
					//g_Log.Add(CLog::Error, "[%s] 66666666", __FUNCTION__);
					if(pUser->pED->EsMago)
					{
						//g_Log.Add(CLog::Error, "[%s] Es MAGO", __FUNCTION__);
						Reuse = pInfo->staticReuseTimeMago;
					}else
					{
						//g_Log.Add(CLog::Error, "[%s] Es WARRIOR", __FUNCTION__);
						Reuse = pInfo->staticReuseTimeWarrior;
					}
				}
			}
		}
...
bool CreatureAction::OnInfectedSkillBy(CCreature *pCreature, CCreature *pSource, CSkillInfo *pInfo)
{
	if(pCreature->ValidCreature() && pInfo)
	{
		...
		if(pInfo->nDebuff == 1 && pCreature->isBlockDebuff && pCreature != pSource)
		{
		...
		}else if(pInfo->nDebuff == 0 && pCreature->isBlockBuff && pCreature != pSource)
		{
...
double CreatureAction::OnGotDamageBy(CCreature *pCreature, CCreature *pAttacker, double Damage, int nDmgType, bool DmgFlag)
{
	...
	if(pCreature->ValidCreature() && pAttacker->ValidCreature())
	{
		...
		if(pAttacker->pSD->pExData->CastedSkill.skill_id)
		{
			...
			if(pData)
			{
				if(pData->soulshotBonus > 0 && pExData->SoulshotActivated && nDmgType != 9)		//nDmgType = 9  :: /TRANFER
				{
					Damage = Damage * pData->soulshotBonus;
				}
				if(pData->magicShieldBlock && nDmgType != 9)
				{
				...
				if(pData->damageBonus != 0 && nDmgType != 9)
				{
					if(pData->weaponCondition > 0)
					{
						//none=0; sword= 1; blunt= 2; dagger= 3; pole= 4; fist= 5; bow= 6; etc= 7; dual=8; dualfist=9; fishing_rod=10;
						if(pData->weaponCondition == pAttacker->pSD->nWeaponType)
						{
		...
		if(g_FrenzyDamageMultipler != 1.0 && nDmgType != 9)
		{
		...
		if(pAttacker->ValidUser())
		{
			if (g_FrenzyDamagePlayerMultipler)
			{
				...
				{
					if(pAttacker->pSD->nIsPC == 1 && pCreature->pSD->nIsPC == 1 && nDmgType != 9)
					{
...
//						g_Log.Add(CLog::Error, "[%d][%d]", wpntype, nWeaponType);
	...
	if(nDmgType != 9)
	{
		if(User *pAttackerUser = pAttacker->SafeCastUser())
		{
			if(User *pUser2 = pCreature->SafeCastUser())
			{
				if(pAttackerUser->pED->EsMago && pUser2->pED->EsMago)
				{
					Damage *= g_MagoVsMago;
				}
				else if(pAttackerUser->pED->EsMago && !pUser2->pED->EsMago)
				{
					Damage *= g_MagoVsWarrior;
				}
				else if(!pAttackerUser->pED->EsMago && pUser2->pED->EsMago)
				{
					Damage *= g_WarriorVsMago;
				}
				else if(!pAttackerUser->pED->EsMago && !pUser2->pED->EsMago)
				{
					Damage *= g_WarriorVsWarrior;
				}


				int nWeaponType = pAttacker->pSD->nWeaponType;
				int wpntype = pAttackerUser->pED->WeapontTypeHand;
				if(pUser2->pED->EsMago)
				{
					if((wpntype == SlotRightLeftHand))
					{
						//none=0; sword= 1; blunt= 2; dagger= 3; pole= 4; fist= 5; bow= 6; etc= 7; dual=8; dualfist=9; fishing_rod=10;
						if(nWeaponType == 0)			Damage *= g_DamagePlayerMultiplerWarriorAMago_noneTwo;
						else if(nWeaponType == 1)		Damage *= g_DamagePlayerMultiplerWarriorAMago_swordTwo;
						else if(nWeaponType == 2)		Damage *= g_DamagePlayerMultiplerWarriorAMago_bluntTwo;
						else if(nWeaponType == 3)		Damage *= g_DamagePlayerMultiplerWarriorAMago_daggerTwo;
						else if(nWeaponType == 4)		Damage *= g_DamagePlayerMultiplerWarriorAMago_poleTwo;
						else if(nWeaponType == 5)		Damage *= g_DamagePlayerMultiplerWarriorAMago_fistTwo;
						else if(nWeaponType == 6)		Damage *= g_DamagePlayerMultiplerWarriorAMago_bowTwo;
						else if(nWeaponType == 7)		Damage *= g_DamagePlayerMultiplerWarriorAMago_etcTwo;
						else if(nWeaponType == 8)		Damage *= g_DamagePlayerMultiplerWarriorAMago_dualTwo;
						else if(nWeaponType == 9)		Damage *= g_DamagePlayerMultiplerWarriorAMago_dualfistTwo;
						else if(nWeaponType == 10)		Damage *= g_DamagePlayerMultiplerWarriorAMago_fishing_rodTwo;
					}
					else
					{
						//none=0; sword= 1; blunt= 2; dagger= 3; pole= 4; fist= 5; bow= 6; etc= 7; dual=8; dualfist=9; fishing_rod=10;
						if(nWeaponType == 0)			Damage *= g_DamagePlayerMultiplerWarriorAMago_none;
						else if(nWeaponType == 1)		Damage *= g_DamagePlayerMultiplerWarriorAMago_sword;
						else if(nWeaponType == 2)		Damage *= g_DamagePlayerMultiplerWarriorAMago_blunt;
						else if(nWeaponType == 3)		Damage *= g_DamagePlayerMultiplerWarriorAMago_dagger;
						else if(nWeaponType == 4)		Damage *= g_DamagePlayerMultiplerWarriorAMago_pole;
						else if(nWeaponType == 5)		Damage *= g_DamagePlayerMultiplerWarriorAMago_fist;
						else if(nWeaponType == 6)		Damage *= g_DamagePlayerMultiplerWarriorAMago_bow;
						else if(nWeaponType == 7)		Damage *= g_DamagePlayerMultiplerWarriorAMago_etc;
						else if(nWeaponType == 8)		Damage *= g_DamagePlayerMultiplerWarriorAMago_dual;
						else if(nWeaponType == 9)		Damage *= g_DamagePlayerMultiplerWarriorAMago_dualfist;
						else if(nWeaponType == 10)		Damage *= g_DamagePlayerMultiplerWarriorAMago_fishing_rod;
					}
				}
				else
				{
					...
					{
						//none=0; sword= 1; blunt= 2; dagger= 3; pole= 4; fist= 5; bow= 6; etc= 7; dual=8; dualfist=9; fishing_rod=10;
						if(nWeaponType == 0)			Damage *= g_DamagePlayerMultiplerWarriorAWarrior_noneTwo;
						else if(nWeaponType == 1)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_swordTwo;
						else if(nWeaponType == 2)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_bluntTwo;
						else if(nWeaponType == 3)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_daggerTwo;
						else if(nWeaponType == 4)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_poleTwo;
						else if(nWeaponType == 5)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_fistTwo;
						else if(nWeaponType == 6)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_bowTwo;
						else if(nWeaponType == 7)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_etcTwo;
						else if(nWeaponType == 8)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_dualTwo;
						else if(nWeaponType == 9)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_dualfistTwo;
						else if(nWeaponType == 10)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_fishing_rodTwo;
					}
					else
					{
						//none=0; sword= 1; blunt= 2; dagger= 3; pole= 4; fist= 5; bow= 6; etc= 7; dual=8; dualfist=9; fishing_rod=10;
						if(nWeaponType == 0)			Damage *= g_DamagePlayerMultiplerWarriorAWarrior_none;
						else if(nWeaponType == 1)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_sword;
						else if(nWeaponType == 2)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_blunt;
						else if(nWeaponType == 3)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_dagger;
						else if(nWeaponType == 4)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_pole;
						else if(nWeaponType == 5)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_fist;
						else if(nWeaponType == 6)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_bow;
						else if(nWeaponType == 7)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_etc;
						else if(nWeaponType == 8)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_dual;
						else if(nWeaponType == 9)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_dualfist;
						else if(nWeaponType == 10)		Damage *= g_DamagePlayerMultiplerWarriorAWarrior_fishing_rod;
					}
				}
...
//	g_Log.Add(CLog::Error, "[%f]", Damage);
	if(nDmgType != 9)
	{
		if(User *pAttackerUser = pAttacker->SafeCastUser())
		{
			if(User *pUser2 = pCreature->SafeCastUser())
			{
				if(!pAttackerUser->pED->EsMago)
				{
					if(!pUser2->pED->EsMago)
					{
						if(Damage > g_DamageWarriorsAWarriorMaximo)
						{
							Damage = g_Random.RandIntMinMax(g_DamageWarriorsAWarriorMinimo,g_DamageWarriorsAWarriorMaximo);
						}
					}
					else
					{
						if(Damage > g_DamageWarriorsAMagoMaximo)
						{
							Damage = g_Random.RandIntMinMax(g_DamageWarriorsAMagoMinimo,g_DamageWarriorsAMagoMaximo);
						}
					}
				}
			}
...
//	g_Log.Add(CLog::Error, "[%f] CRITIAL", Damage);
