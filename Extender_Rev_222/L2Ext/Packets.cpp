...
//CPET
	...
	//DiePacket
	g_HookManager.WriteCall(0x8293BE, DiePacket, 0);

#ifdef CHANGE_OPCODES
	//Test Fix For Classic
	g_HookManager.WriteCall(0x007D5FCA, ValidateSend, 0); //CSocket::Send
	g_HookManager.NopMemory(0x87732D, 8); //nop this memory
#endif
	
	//7D5E1E -> hook for change opcodes
...
void CPacketFix::ValidateSend(CSocket *pSocket, const char *format, va_list va)
{
	try
	{
		...
		BYTE nPacketID = va_arg(va, BYTE);

		if(CUserSocket *pUS = pSocket->CastUserSocket())
		{
			if(pUS->pED->is_osr || pUS->pED->is_osr_buff)
			{		
				/*if(pUS->pED->osr_buff->current_stage == -1)
				{
					typedef void (*f)(CSocket*, const char *, va_list);
					f(0x7D5D40L)(pSocket, format, va);
				}*/
				g_Log.Add(CLog::Error, "[%s] Packet Not Sended[%x]!", __FUNCTION__, nPacketID);
			}
			else
			{
				typedef void (*f)(CSocket*, const char *, va_list);
				...
				g_Log.Add(CLog::Blue, "[%s] Packet Sended[%x]!", __FUNCTION__, nPacketID);
			}
		}
		else
		{
