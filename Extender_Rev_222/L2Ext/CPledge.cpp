...
int CPledge::GetAllianceCounts()
{
	try
	{
		INT64 dAllianceID = (INT64)GetAlliance();
		if(dAllianceID==NULL) return 0;
		dAllianceID += 0x50;
		int count = *(INT64*)dAllianceID;
...
int CPledge::GetAllianceMasterPledgeID()
{
	try
	{
		INT64 dAllianceID = (INT64)GetAlliance();
		...
		dAllianceID = *(INT64*)dAllianceID;
		if(dAllianceID == NULL) return NULL;
		dAllianceID += 0x98;			//m_nMasterPledgeDbId
		return *(DWORD*)dAllianceID;
	}
	catch(...)
	{
...
/*
void CPledge::GetAllMemberAlliancePledge(MemberDataAllyMap& lista) 	///CAlliance
{	
	guard;

	NT64 pAlly = (INT64)GetAlliance();
	typedef void (*__up)(INT64, MemberDataAllyMap&);
	__up _up = (__up) 0x00421FE0;
	_up(pAlly, lista);
...
MemberDataAllyMap CPledge::GetAllMemberAlliancePledge()
{
	INT64 pAlly = (INT64)GetAlliance();
	typedef void (__fastcall *__up)(INT64, void*);
	__up _up = (__up) 0x00421FE0;
	BYTE pVectorBuff[64];
	memset(pVectorBuff, 0x00, 64);
	_up(pAlly, pVectorBuff);
	
	return *(MemberDataAllyMap*)&pVectorBuff; 
}



int CPledge::GetDbMemberCount()
{
	typedef int (__fastcall *__up)(CPledge*);
	__up _up = (__up) 0x0046E390;
	return _up(this);
}
...
{	
	...
	if(NewAllySystem)
	{
		...
		if(ally != NULL)
		{
			...
			MemberDataAllyMap vEnemy = GetAllMemberAlliancePledge();
			for(Iter=vEnemy.begin();Iter!=vEnemy.end();Iter++)
			{
				CPledge *pTmp = *Iter;
				if(pTmp)
			...
			if (AllytotalPlayers > g_ClanMembersLimit.MainClan5)
			{
	//			CSystemMessage msg1(SystemMessage::S1_POPUP);
	//			msg1.AddText(L"No pueden agregar miembros.\nLa Ally esta completa.");
	...
	case MainClan:
		{
			...
			{
				if(nMembers < g_ClanMembersLimit.MainClan4)
					return true;
			}else if(nLevel < 6)
			{
				if(nMembers < g_ClanMembersLimit.MainClan5)
					return true;
			}else if(nMembers < g_ClanMembersLimit.MainClan6)
				return true;
			break;
		}
	case Academy:
