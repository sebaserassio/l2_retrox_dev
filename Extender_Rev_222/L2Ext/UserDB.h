...
public:
	...
	void KickOutFromRangeUser(User* pUser, IVector pos, int minX, int maxX, int minY, int maxY);
	bool AnyPrivateStoreInRange(FVector pos, double range);
	bool AnyBuffStoreInRange(FVector pos, double range);
	void GiveItemToAllUser(INT32 itemId, INT32 count, INT32 enchant=0, BOOL checkIp=FALSE);
	void GiveSkillToAllUser(INT32 skillId, INT32 level);
