...
{
	wstringstream file(ReadFileW(g_ChangeFolders.VisualArmor_txt));
	wstring line;
	m_Lock.Enter();
	m_Armors.clear();
	while(getline(file, line))
	{
		...
		}else if(line.find(L"armor_begin") == 0)
		{
			...
			pV->hairSlotId = Parser::ParseOptionInt(line, L"slot_hair");
			...
			pV->itemVerify = Parser::ParseOptionInt(line, L"item_verify");
			pV->item_icon = Parser::ParseOptionString(line, L"item_icon");

			//g_Log.Add(CLog::Error, "[%s] Slot Hair[%d]!", __FUNCTION__, pV->price.itemId);
			//g_Log.Add(CLog::Error, "[%s] Slot Hair[%d]!", __FUNCTION__, pV->hairSlotId);
			...
			if(startPos != wstring::npos)
			{
				...
				if(endPos != wstring::npos)
				{
					...
					for(map<UINT, VisualArmorInfo*>::iterator it = m_Armors.begin(); it!=m_Armors.end();it++)
					{
						...
						if(pUser->inventory.HaveItemByClassID(it->second->itemVerify, 1))
						{
						...
						line = Utils::ReplaceString(line, L"$item_icon", it->second->item_icon, true);
						...
						if (cont == 1)
						{
							line = Utils::ReplaceString(line, L"$tr_in", L"<tr>", true);
							line = Utils::ReplaceString(line, L"$tr_out", L"", true);
						}
						else if(cont == 2)
						{
							line = Utils::ReplaceString(line, L"$tr_in", L"", true);
							line = Utils::ReplaceString(line, L"$tr_out", L"</tr>", true);
							cont=0;
						}
						else
						{
...
bool CVisualArmor::OnTradeTest(UINT itemId, UINT armorId)
{
			if(VisualArmorInfo* pV = Get(armorId))
			{
				if(pV->itemVerify == itemId)
				{
					return false;
				}
			}
			...
			if(VisualArmorInfo* pV = Get(armorId))
			{
				if(pUser->pSD->nLevel >= pV->minLevel)
				{
					if(pUser->pSD->nLevel <= pV->maxLevel)
					{
						...
						if(pUser->inventory.HaveItemByClassID(pV->itemVerify, 1))
						{
						...
						else
						{
							...
							{
								if(pUser->inventory.CheckAddable(pV->itemVerify, 1))
								{
									...
									pUser->AddItemToInventory(pV->itemVerify, 1);
								}
								else
								{
									pUser->SendSystemMessage(L"Necesitas mas espacio en el inventario!");
									unguard;
									return;
								}
						...
						vUser.hairSlotId = pV->hairSlotId;
...
void CVisualArmor::OnEquipTest(User *pUser, UINT armorId)
{
	guard;
	if(m_Enabled)
	{
		//if(pUser->pED->visualArmorUser.floodTimestamp_test < time(0))
		//{
			if(pUser->olympiadUser.GetState() != 0)
			{
				pUser->SendSystemMessage(L"You cannot perform this action while participing in Olympiad!");
			...
			if(VisualArmorInfo* pV = Get(armorId))
			{
				pUser->pED->visualArmorUser.floodTimestamp_test = time(0) + 10;
				VisualArmorUser& vUser = pUser->pED->visualArmorUser;
				vUser.armorId = armorId;
				vUser.armorType = pV->armorType;
				vUser.chestSlotId = pV->chestSlotId;
