...
extern INT32 g_DropSpoilEnchantArmorMaxTimeMinutes;
extern INT32 g_DropSpoilEnchantWeaponMaxTimeMinutes;
extern INT32 g_DropSpoilEnchantArmorContadorDrop;
extern INT32 g_DropSpoilEnchantWeaponContadorDrop;
...
void CL2Time::TimerExpired(CL2Time* pL2Time, int nID)
{
	...
	// p.ej. en tu hook de IOThread_common o en un timer del engine:
	g_ChatMobile.PumpFromEngine();
	...
	if(nID == 1)	//tiggers every 1000 ms
	{
		if(!g_BlockedLicense)
		{
			...
			g_TvT.TimerExpired();
			g_RatesDinamicos.TimerExpired();

			g_NpcSpawnEvent.TimerExpired();
			g_PvPRankEvent.TimerExpired();
			...
			g_OlympiadStatus.TimerExpired();
			g_ClanPvPStatus.TimerExpired();
			g_EpicItems.TimerExpired();
			g_DailyPvP.TimerExpired();
			g_ChatManager.TimerExpired();
			g_DBNpcMaker.TimerExpired();

			g_ClanReputationRank.TimerExpired();

//			g_OfflineShopRestore.TimerExpiredSrv();

			//g_Win2k12.TimerExpired();

			g_PvPEvents.TimerExpired();

			g_PartyEvents.TimerExpired();
			...
			if(g_DropSpoilEnchantArmorMaxTimeMinutes > 0)
			{
				if((currentTime % g_DropSpoilEnchantArmorMaxTimeMinutes) == 0)
				{
					g_DropSpoilEnchantArmorContadorDrop = 0;
				}
			}

			if(g_DropSpoilEnchantWeaponMaxTimeMinutes)
			{
				if((currentTime % g_DropSpoilEnchantWeaponMaxTimeMinutes) == 0)
				{
					g_DropSpoilEnchantWeaponContadorDrop = 0;
				}
			}
