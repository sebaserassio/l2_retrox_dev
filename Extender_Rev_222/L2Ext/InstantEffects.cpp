...
extern BOOL g_CelestialDisAggro;

extern BOOL m_desactivar_death2;
...
void CSkillEffect_i_update_eval_score::Instant(CCreature *pCreature, CCreature *pTarget, CSkillInfo *pSkillInfo, CSkillAction *pAction, double distance)
{
	if(User *pUser = pCreature->SafeCastUser())
	{
		...
		f(0x530F70L)(0x10488E0L, pUser->nDBID, amount, pUser->pSD->m_nSulffrageUsedCount, &st);
	...
	if(User *pUser = pCreature->SafeCastUser())
	{
		if(pUser->pSD->nPledgeID)
		{
			if(pUser->pSD->nIsPledgeLeader)
			{
				if(CPledge *pPledge = pUser->GetPledge())
				{
					pUser->DeleteItemInInventory(9642, 1);
...
void CSkillEffect_i_report_bot::Instant(CCreature *pCreature, CCreature *pTarget, CSkillInfo *pSkillInfo, CSkillAction *pAction, double distance)
{
	if(User *pUser = pCreature->SafeCastUser())
	{
		//Report User As Bot
...
void CSkillEffect_i_death2::Instant(CCreature *pCreature, CCreature *pTarget, CSkillInfo *pSkillInfo, CSkillAction *pAction, double distance)
{
	if(pCreature->ValidCreature() && pTarget->ValidCreature())
	{
		//LETHAL no le deja tirar a estos mobs, yo pondria que no le entre lethal a ningun MOB
		//agregando !pTarget->IsNPC()
		//[tyrannosaurus1] = 1022215
		//[tyrannosaurus2] = 1022216
		//[tyrannosaurus3] = 1022217

		if(!pTarget->IsZZoldagu() && !pTarget->IsBoss() && pTarget->canDamageHP && !m_desactivar_death2 && 
			pTarget->pSD->nNpcClassID != 1022215 && pTarget->pSD->nNpcClassID != 1022216 && pTarget->pSD->nNpcClassID != 1022217)
		{
...
void CSkillEffect_i_cancel_min_max::Instant(CCreature *pSpeller, CCreature *pTarget, CSkillInfo *pSkillInfo, CSkillAction *pAction, double distance)
{
	if(pSpeller->ValidCreature() && pTarget->ValidCreature())
	{
		int nCount = 10; //Max buffs/debuffs that can be removed
		int maximo = (int)dCount; //Max buffs/debuffs that can be removed
		int nType = 0;	//solo los buff
		int minimo = (int)dType;
		int quitados = 0;

		int total = 0;
		total = g_Random.RandIntMinMax(minimo,maximo);
		...
		if(pSpeller->ValidCreature() && pTarget->ValidCreature())
		{
			...
			if(!bBlocked && pSkillInfo)
			{
				...
				pTarget->abnormalStatusLock.Lock(__FILEW__, __LINE__);
					...
					int contarBuffs = 0;
					AbnormalDataVector::iterator IterTest=pTarget->vAbnormalStatus.begin();

					for(; IterTest != pTarget->vAbnormalStatus.end(); IterTest++)
					{
						AbnormalData BI = *IterTest;
						if(BI.pSI)
						{
							if(BI.pSI->nResistCancel)
							{
...
inicio:
					...
					if(bDispell)
					{
						...
						{
							if(pTU->pED->inZoneCustomBuffBackup)
							{
								int AbnormalTime = pSkillInfo->nAbnormalTime;
								if(pSkillInfo->nSkillID == 342)			//[s_touch_of_death1]	skill_id=342
								{
									AbnormalTime = 20;
								}
								pTU->HideAbnormal((*Iter), AbnormalTime);
							}
						}
					...
					if ( contarBuffs>0 && quitados < total && contarBuffs-quitados>0 && contador<50)
							goto inicio;
...
void CSkillEffect_i_dispel_by_slot_probability_backup::Instant(CCreature *pSpeller, CCreature *pTarget, CSkillInfo *pSkillInfo, CSkillAction *pAction, double distance)
{
	if(pSpeller->ValidCreature() && pTarget->ValidCreature())
	{
		int Abnormal = (int)dAbnormal;
		...
		for(; Iter != pTarget->vAbnormalStatus.end(); Iter++)
		{
			...
			if(BI.pSI)
			{
				if(BI.pSI->nAbnormalType == Abnormal)
				{

					INT32 rnda = g_Random.RandInt(100);
...
void CSkillEffect_i_cancel_min_max_backup::Instant(CCreature *pSpeller, CCreature *pTarget, CSkillInfo *pSkillInfo, CSkillAction *pAction, double distance)
{
	if(pSpeller->ValidCreature() && pTarget->ValidCreature())
	{
		int nCount = 10; //Max buffs/debuffs that can be removed
...
void CSkillEffect_i_dispel_by_category_rand::Instant(CCreature *pSpeller, CCreature *pTarget, CSkillInfo *pSkillInfo, CSkillAction *pAction, double distance)
{
	if(pSpeller->ValidCreature() && pTarget->ValidCreature())
	{
		int nCount = rand() % (int)dCount + 1;//(int)dCount; //Max buffs/debuffs that can be removed
		int nType = (int)dType;
		double dChance = 250;			//Base chance
...
void CSkillEffect_i_dispel_by_category_rand50::Instant(CCreature *pSpeller, CCreature *pTarget, CSkillInfo *pSkillInfo, CSkillAction *pAction, double distance)
{
	if(pSpeller->ValidCreature() && pTarget->ValidCreature())
	{
		int nCount = rand() % (int)dCount + 1;//(int)dCount; //Max buffs/debuffs that can be removed
...
void CSkillEffect_i_spawn_aganthion::Instant(CCreature *pCreature, CCreature *pTarget, CSkillInfo *pSkillInfo, CSkillAction *pAction, double distance)
{	
	if(pCreature->ValidUser())
	{
		int nClassID = dNpcID;
		...
		CNPC *pAganthion = pCreature->GetUser()->pSD->pExData->Aganthion;
		if(pAganthion->ValidCreature())
		{
			...
			b(0x4D4570)(pCreature->pSD->pExData->Aganthion);

			if(pCreature->HaveSkill(nSkillID, 1))
			{
				pCreature->DeleteSkill(nSkillID);

				typedef char (*f)(CCreature*);
				f(0x5173F0)(pCreature);	//CCreature::ValidateAttachedSkills
				pCreature->GetUser()->ValidateAllOnSIM();
			}

			pCreature->GetUser()->pSD->pExData->Aganthion = NULL;
		...
		CNPC *pNPC = g_ObjectDB.CreateCSummon(nClassID);


				//bool __stdcall User::SummonCreature(User *this, const WCHAR *pszName, int type, const WCHAR *pszItemName, int count)
//				typedef bool (*h)(User*, const WCHAR *, int, const WCHAR *, int);
//				bool ret = h(0x81F5F0L)(pCreature->GetUser(), L"Agathion_34017",1, L"adena", 1);
		...
		//		pNPC->WriteLock(__FILEW__, __LINE__);
				...
				pNPC->pSD->nMasterID = pCreature->pSD->nObjectID;	//1920; m_SummonerID,summoner_id	(en L2NPC AI)
				pNPC->pSD->nPrivateID = pCreature->pSD->nPrivateID;

				pNPC->pSD->nMakerParam3 = nSkillID;

				//g_Log.Add(CLog::Error, "[%s] nSkillID[%d]", __FUNCTION__, nSkillID);
				pCreature->GetUser()->AcquireSkill(nSkillID, 1);
	...
	//			}
				...
				A(0x74B380)(&pNPC->pSD->_n0760, pCreature->GetUser());

			pNPC->pSD->nSummonType = 3;
			pNPC->pSD->_n0770 = 1;
			pNPC->pSD->_n0784 = pCreature->pSD->nObjectID;

            pNPC->pSD->_n077C = 1;	//type


			(*(LPDWORD)((UINT64)pNPC+0x1A58)) = pCreature->GetUser()->nDBID;

			(*(LPDWORD)((UINT64)pNPC+0x1A5C)) = pCreature->pSD->nObjectID;
...
//pNPC->canMove = true ;
	...
	//			typedef bool (*A)(INT64, User*, CNPC*);
	//			A(0x753370)(0xE1F9850, pCreature->GetUser(), pNPC);
		...
		//			pNPC->WriteUnlock();
				...
				{
					pCreature->pSD->pExData->Aganthion = pNPC;
					pNPC->pSD->fSpeed = pCreature->pSD->fSpeed;
					pNPC->pSD->m_nMoveTypeSpeed = pCreature->pSD->m_nMoveTypeSpeed;
//
					pNPC->_unkn6C0 = pCreature->_unkn6C0;
					pNPC->_n500[29] = pCreature->_n500[29];
					pNPC->speedPER = pCreature->speedPER;
					pNPC->f_area_speed_coef = pCreature->f_area_speed_coef;
					pNPC->carry_weight_speed_coef = pCreature->carry_weight_speed_coef;
					pNPC->speedDIFF = pCreature->speedDIFF;

					typedef double (*f)(INT64, int, int, int, int);
					pNPC->pND->run_speed = f(0x744B80)(0xE1F3430, pCreature->pSD->nSex, pCreature->pSD->nClass, 1, 0);
					pNPC->pND->walk_speed = f(0x744B80)(0xE1F3430, pCreature->pSD->nSex, pCreature->pSD->nClass, 0, 0);
					pNPC->pND->swim_run_speed = f(0x744B80)(0xE1F3430, pCreature->pSD->nSex, pCreature->pSD->nClass, 1, 1);
					pNPC->pND->swim_walk_speed = f(0x744B80)(0xE1F3430, pCreature->pSD->nSex, pCreature->pSD->nClass, 0, 1);
					pNPC->pND->fly_run_speed = f(0x744B80)(0xE1F3430, pCreature->pSD->nSex, pCreature->pSD->nClass, 1, 3);
					pNPC->pND->fly_walk_speed = f(0x744B80)(0xE1F3430, pCreature->pSD->nSex, pCreature->pSD->nClass, 0, 3);
					pNPC->pND->fly_run_speed2 = f(0x744B80)(0xE1F3430, pCreature->pSD->nSex, pCreature->pSD->nClass, 1, 2);
					pNPC->pND->fly_walk_speed2 = f(0x744B80)(0xE1F3430, pCreature->pSD->nSex, pCreature->pSD->nClass, 0, 2);
...
//pCreature->GetUser()->walk_speed = pCreature->GetUser()->run_speed;
...
void CSkillEffect_i_target_cancel_chance::Instant(CCreature *pCreature, CCreature *pTarget, CSkillInfo *pSkillInfo, CSkillAction *pAction, double distance)
{
	if(pCreature->ValidCreature() && pTarget->ValidCreature())
	{
		if(!pTarget->IsBoss() && !pTarget->IsZZoldagu())
		{
			...
			if(LandRate < dChance)
			{
				if(!pTarget->isBlockDebuff && g_CelestialDisAggro)
				{
...
void CSkillEffect_i_give_item_maxcount::Instant(CCreature *pCreature, CCreature *pTarget, CSkillInfo *pSkillInfo, CSkillAction *pAction, double distance)
{
	guard;
	if(User *pUser = pCreature->GetUser())
	{
		...
		if(pUser->inventory.CheckAddable(itemId, count))
		{
			...
			CItem *pItem = pUser->GetInventory()->GetFirstItemByClassID(itemId);

			if(pItem->IsValidItem())
			{

				//pUser->SendSystemMessage(L"pItem");
				int nuevoCount = count - pItem->pSID->nItemAmount;

				//g_Log.Add(CLog::Error, "itemId2[%d] nuevoCount[%d]", (int)itemId, (int)nuevoCount);
				if(nuevoCount > 0)
				{
					pUser->AddItemToInventory(itemId, nuevoCount);
					g_Logger.Add(L"User[%s] get item[%d][%d] - skill effect i_give_item_maxcount", pUser->pSD->wszName, (int)itemId, nuevoCount);
				}
			}
			else
			{
				//pUser->SendSystemMessage(L"normal");
				pUser->AddItemToInventory(itemId, count);
				g_Logger.Add(L"User[%s] get item[%d][%d] - skill effect i_give_item_maxcount", pUser->pSD->wszName, (int)itemId, (int)count);
			}
		}
...
void CSkillEffect_i_sweep_consume_body::Instant(CCreature *pCreature, CCreature *pTarget, CSkillInfo *pSkillInfo, CSkillAction *pAction, double distance)
{
	if(pCreature->ValidCreature() && pTarget->ValidCreature())
	{
		CSkillOperateCondition_op_sweep_spoil* pCond = new CSkillOperateCondition_op_sweep_spoil;

		typedef bool (*f) (CSkillOperateCondition*, CCreature*, CObject*);
		bool bOpConsumeBody = f(0x7CC5E0)(pCond, pCreature, pTarget);
		bool bOpSweeper = f(0x7CE210)(pCond, pCreature, pTarget);
		
		typedef void (*g) (CSkillEffect *, CCreature *, CCreature *, CSkillInfo *, CSkillAction *, double);

		if(bOpConsumeBody && bOpSweeper)
		{
			g(0x7D2E70)(this, pCreature, pTarget, pSkillInfo, pAction, distance);
			g(0x7AE7D0)(this, pCreature, pTarget, pSkillInfo, pAction, distance);
		}
	}
}


void CSkillEffect_i_dispel_skill_id::Instant(CCreature *pCreature, CCreature *pTarget, CSkillInfo *pSkillInfo, CSkillAction *pAction, double distance)
{
	guard;
	if(pCreature->ValidCreature() && pTarget->ValidCreature())
	{
		UINT skillId = static_cast<UINT>(m_skillId);
		UINT ab_level = static_cast<UINT>(m_level);
		if(skillId > 0)
		{

			bool validate = false;
			pTarget->abnormalStatusLock.Lock(__FILEW__, __LINE__);
			for(AbnormalDataVector::iterator it = pTarget->vAbnormalStatus.begin();it!=pTarget->vAbnormalStatus.end();it++)
			{
				if(it->pSI)
				{
					if(it->pSI->nSkillID == skillId)
					{
						for(UINT n=0;n<40;n++)
						{
							if(pTarget->pSD->AbnormalTable[n][0] == it->pSI->nAbnormalType && pTarget->pSD->AbnormalTable[n][1] == ab_level)
							{
								pTarget->pSD->AbnormalTable[n][0] = -1;
								break;
							}
						}
						validate = true;
