...
{
	//g_Log.Add(CLog::Error, "[%s] 1111111111", __FUNCTION__);
	if(pUser->ValidUser())
	{
		Lock();
		try
...
User* CUserDB::GetUserByDBID(UINT databaseId)
{
	...
	{
		Lock();
		//g_Log.Add(CLog::Error, "[%s] databaseId[%d]", __FUNCTION__, databaseId);
		map<UINT, User*>::iterator Iter = m_byDatabaseId.find(databaseId);
		if(Iter!=m_byDatabaseId.end())
		{
			//g_Log.Add(CLog::Error, "[%s] databaseId[%d] pUser[%d]", __FUNCTION__, databaseId, pUser);
			pUser = Iter->second;
		}
		Unlock();
	}
...
void CUserDB::KickOutFromRangeUser(User* pUser, IVector pos, int minX, int maxX, int minY, int maxY)
{
	double x1 = static_cast<double>(minX);
	double x2 = static_cast<double>(maxX);
	double y1 = static_cast<double>(minY);
...
bool CUserDB::AnyBuffStoreInRange(FVector pos, double range)
{
	bool ret = false;
	guard;
	Lock();
...
void CUserDB::GiveItemToAllUser(INT32 itemId, INT32 count, INT32 enchant, BOOL checkIp)
{
	guard;
	Lock();
	try
					...
					if(pUser->pSD->activateOfflineShopTime == 0)
					{
						...
						if(canReceiveReward)
						{
							...
							if(pUser->pSD->activateOfflineShopTime == 0 )
							{
								...
								if(pUser->inventory.CheckAddable(itemId, count) && pUser->IsNowTrade() == false && pUser->pSD->nStoreMode == 0)
									pUser->AddItemToInventory(itemId, count, false, enchant);
								else
									g_DB.RequestItemDeliveryAdd(pUser->pSD->nDBID, itemId, count, enchant, 0);
							}
						}else
						{
...
void CUserDB::GiveSkillToAllUser(INT32 skillId, INT32 level)
{
	guard;
	Lock();
	try
					...
					if(pUser->pSD->activateOfflineShopTime == 0)
					{
						...
						if(canReceiveReward)
						{
							if(pUser->pSD->activateOfflineShopTime == 0 )
							{
								...
								{
									pUser->AcquireSkill(skillId, level, TRUE);
									pUser->SendAcquireSkillToDB(skillId, level, true);
								}
							}
						}else
						{
							g_Logger.Add(L"User[%s] cannot receive skill reward[%d][%d]", pUser->pSD->wszName, skillId, level);
						}
					}
				}
			}
...
map<UINT, User*> CUserDB::GetAllUser()
{
	return m_byServerId;
}
