...
void CItemDelivery::HandleRequest(User *pUser, UINT jobId, INT32 itemId, INT32 amount, INT32 enchant, UINT augmentation, UINT auction, UINT sold_id, UINT sold_amount, WCHAR* wBuyer)
{
	guard;
	...
	if(pUser->ValidUser())
	{
		...
		{
			pUser->AddItemToInventory(itemId, amount, false, enchant, 0, 0, augmentation);

			if(auction == 1) //Payment - Sold
			{
				g_Auction.HandlePaymentRequest(pUser, itemId, amount, sold_id, sold_amount, wBuyer);
			}
			else // Normal Item Delivery
			{
