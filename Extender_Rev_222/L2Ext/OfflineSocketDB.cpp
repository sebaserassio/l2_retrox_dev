...
bool COfflineSocketDB::Add(CUserSocket *pSocket, SOCKET s/*, CIOObject *pIOObject*/)
{
	Lock();
	try
	{
...
bool COfflineSocketDB::StartCloseTimer(CUserSocket *pSocket)
{
	...
	try
	{
		//g_Log.Add(CLog::Error, "[%s] did it50!", __FUNCTION__);
		OfflineSocketMap::iterator Iter = mData.find(pSocket);
		if(Iter!=mData.end())
		{
			...
			/*pSocket->s = Iter->second.s;
			pSocket->Close();
			*/

			//pSocket->closeTime = _time32(NULL);
			//g_Log.Add(CLog::Error, "[%s] did it!", __FUNCTION__);
