...
bool CSkillFx::OnCreatureValidateMUseSpeed(CCreature *pCreature)
{
	...
	if(pCreature)
	{
		...
		if(User *pUser = pCreature->SafeCastUser())
		{
			...
			{
				if(pCreature->pSD->castingSpeed > 6.306306306306306)
				{
					pCreature->pSD->castingSpeed = 6.306306306306306;
				}
			}
		}
...
bool CSkillFx::OnCreatureValidateAttackSpeed(CCreature *pCreature)
{
	...
	if(pCreature)
	{
		...
		if(User *pUser = pCreature->SafeCastUser())
		{
			if(pUser->pED->aioId != 0)
			{
				if(pCreature->pSD->attackSpeed > 1800)
				{
					pCreature->pSD->attackSpeed = 1800;
	...
	if(User *pUser = pCreature->SafeCastUser())
	{
		if(pTarget->ValidUser() || pTarget->IsPet())
		{
			if(pTarget->pSD->nAlive == 0)
			{
				...
				if(User *ppTarget = pTarget->SafeCastUser())
				{
					time_t now = time(0);
					if(ppTarget->pED->m_TimeResTeleport > now && ppTarget->pED->nInCustomEventClanZone)
					{
						int totaltiempo = ppTarget->pED->m_TimeResTeleport - now;
						std::wstringstream wss;
						wss << "El personaje " << ppTarget->pSD->wszName <<" tiene penalizacion de res por " << totaltiempo << " segundos.";
						...
						wss1 << "Tienes penalizacion de res por " << totaltiempo << " segundos.";
						wstring value4 = wss1.str();
						const WCHAR * value3 = value4.c_str();

						CSystemMessage msg2(2907);
						msg2.AddText(value3);
						ppTarget->SendSystemMessage(&msg2);
				...
				if(User *pTargetU = pTarget->SafeCastUser())
				{
					TerritoryData& td = pUser->pSD->pExData->territoryData;
					TerritoryData& td2 = pTargetU->pSD->pExData->territoryData;
						
					if((td.territoryId[FACTION_ZONE] != 0) && (td2.territoryId[FACTION_ZONE] != 0))
					{
						if( pTargetU->pED->nFactionID != pUser->pED->nFactionID )
						{
							pUser->SendSystemMessage(L"You cannot resurrect a player who belongs on other faction!");
							unguard;
							return;
						}
					}
