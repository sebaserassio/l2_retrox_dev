...
void NpcAction::Initialize()
{
	...
	}else
	{
		...
		g_EpicNpcs.push_back(1029006);	//core
		...
		//g_EpicNpcs.push_back(1029046);	//follower of frintezza - not needed for now
	...
	if(threadIndex < 26)
	{
		...
		if(pUser->ValidUser())
		{
			if(itemId == 57 || itemId == 6360 || itemId == 6361 || itemId == 6362)
			{
				...
				adenaCount *= g_HappyHours.GetAdenaRate(pUser);
...
void NpcAction::EnterWorld(CNPC* pNPC, bool bSetDefaultParam, int nHP, int nMP)
{
	...
	if(pNPC->ValidCreature())
	{
		...
		if(pNPC->IsBoss())
		{
			...
			g_RaidBossStatus.OnNpcSpawn(pNPC);



			g_TerritoryChecker.AddZonePvP(npcId, pNPC->pSD->spawnPos.x, pNPC->pSD->spawnPos.y, pNPC->pSD->spawnPos.z);
			//g_TerritoryChecker.DeleteZonePvP2(1025325);
		}
...
bool NpcAction::OnDie(CNPC *pNPC, CCreature *pEnemy)
{
	...
	if(pNPC->ValidCreature())
	{
		...
		g_Captcha.OnDie(pEnemy,pNPC);
		g_RaidBossStatus.OnNpcDie(pNPC);

		if(pNPC->IsBoss())
		{
			INT32 npcClassId = pNPC->pSD->nNpcClassID;
			g_AutoAnnounce.OnBossKillWorld(npcClassId, pEnemy);

			g_TerritoryChecker.DeleteZonePvP(npcClassId);
		}

		g_TvT.IsNpcDestroyTheBaseDIE(pNPC->nObjectID);
	}
	

//		g_DropRaidsCustom.Handle(pNPC, pEnemy);
//		g_Achivments.SetUser_KillMob(pNPC, pEnemy);
