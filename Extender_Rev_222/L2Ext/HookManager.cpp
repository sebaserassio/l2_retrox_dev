...
void CHookManager::WriteMemoryBYTES(uintptr_t addr, const void* data, size_t len)
{
	DWORD OLDPROTECT;
	SIZE_T BW;
...
void CHookManager::WriteMemoryBYTES(uintptr_t addr, const char* text)
{
...
void CHookManager::WriteMemoryBYTES(uintptr_t addr, size_t len, ...)
{
	if (len <= 0) return;
...
void CHookManager::WriteMemoryBYTE2(UINT64 Address, BYTE Value, LPBYTE OrgBytes)
{
	if(Address)
	{
		SIZE_T Size = sizeof(BYTE);
