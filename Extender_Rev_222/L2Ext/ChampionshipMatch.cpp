...
void CMatch::Broadcast(Championship::TeamType team, const char *format, ...)
{
	...
	for(map<UINT, User*>::iterator Iter = m_users.begin();Iter!=m_users.end();Iter++)
	{
		...
		if(pUser->ValidUser())
		{
			if(team == Championship::TeamNone || pUser->pED->championshipUser.teamType == team)
			{
...
User* CMatch::AddUser(UINT charId, Championship::TeamType team)
{
	...
	if(User *pUser = g_UserDB.GetUserByDBID(charId))
	{
		if(pUser->ValidUser())
		{
			pUser->pED->championshipUser.matchId = id;
			pUser->pED->championshipUser.teamType = team;
			pUser->pED->championshipUser.state = Championship::UserTeleporting;
			m_users.insert(pair<UINT, User*>(pUser->nDBID, pUser));
			return pUser;
		}
	}
	return 0;
...
void CMatch::OnFinish(INT32 rewardId, INT32 rewardCount)
{
	...
	for(map<UINT, User*>::iterator userIter = m_users.begin(); userIter != m_users.end(); userIter++)
	{
		...
		if(pUser->ValidUser())
		{
			pUser->pED->championshipUser.state = Championship::UserTeleportingBack;
			pUser->SendUserInfo();
			pUser->SendCharInfo();
			UINT teamType = pUser->pED->championshipUser.teamType;
