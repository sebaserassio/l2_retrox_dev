...
void CCreatureTimerHandler::HandleMoveAganthion(CCreature *pCreature)
{
			if(pCreature->pSD->nNpcClassID >= 1033910 && pCreature->pSD->nNpcClassID <= 1033928)
			{

				User *pMaster = pCreature->GetMaster();
				if(pMaster->ValidUser())
				{
//					g_Log.Add(CLog::Blue, "000000000000");
...
//						g_Log.Add(CLog::Blue, "111111111111111");
						pCreature->pCreatureController->AsyncMoveToTarget(pMaster->CastCreature(),0);
					}

					AddTimer(MOVE_AGANTHION, 0);
				}
				else
				{
					pCreature->CastNPC()->TeleportToLocation(0,0,0,0);
//						g_Log.Add(CLog::Blue, "2222222222222");
					typedef double (*b)(CNPC*);
					b(0x4D4570)(pCreature->CastNPC());
				}
			}
}
...
void CCreatureTimerHandler::HandleRbFlag(CCreature *pCreature)
{
