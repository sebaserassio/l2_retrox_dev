...
void CNpcDb::LoadData()
{
	...
	while(getline(npcDataStream, line))
	{
		...
		if(line.find(L"npc_begin") == 0)
		{
			...
			if(dataStream >> temp >> temp >> id >> name)
			{
				...
				ni.name = name;
				m_DB.push_back(ni);
				m_DB2.push_back(ni); //For Second DB
			}
		}
	}

	//load NpcName.txt
	wstringstream nameStream(ReadFileW(g_ChangeFolders.NpcName_txt));
	...
	//load NpcName2.txt
	wstringstream nameStream2(ReadFileW(g_ChangeFolders.NpcName2_txt));
	while(getline(nameStream2, line))
	{
		//20001	Gremlin		9C	E8	A9	-1
		if(line.size() > 0 && line.find(L"/") != 0)
		{
			...
			if(cols.size() > 1)
			{
				...
				if(id > 0 && clientName.size() > 0)
				{
					for(UINT n=0;n<m_DB2.size();n++)
					{
						if(m_DB2[n].id == id)
						{
							m_DB2[n].clientName = clientName;
							break;
						}
					}
				}
...
wstring CNpcDb::GetDbNameByClassId(UINT classId)
{
	guard;
	wstring npcDBname = L"";
	for(UINT n=0;n<m_DB.size();n++)
...
wstring CNpcDb::GetClientNameByName2(std::wstring name)
{
	guard;
	wstring clientName = L"";
	for(UINT n=0;n<m_DB2.size();n++)
	{
		if(m_DB2[n].name == name)
		{
			clientName = m_DB2[n].clientName;
			break;
		}
	}
	unguard;
...
wstring CNpcDb::GetClientNameByClassId2(UINT classId)
{
	guard;
	wstring clientName = L"";
	for(UINT n=0;n<m_DB2.size();n++)
	{
		if(m_DB2[n].classId == classId)
		{
			clientName = m_DB2[n].clientName;
			break;
		}
