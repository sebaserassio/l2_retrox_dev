...
bool CKillDb::IsInReuse(UINT killerId, UINT victimId, UINT reuse, bool war)
{
	guard;
	bool ret = false;
	unguard;
	...
	{
		map<UINT, vector<KillData>>::iterator it = m_DataWar.find(killerId);
		if(it!=m_DataWar.end())
		{
			vector<KillData>& vKillData = it->second;
			bool found = false;
		...
		}else
		{
			...
			kd.killTime = time(0);
			vKillData.push_back(kd);
			m_DataWar.insert(pair<UINT, vector<KillData>>(killerId, vKillData));
		}
	}
