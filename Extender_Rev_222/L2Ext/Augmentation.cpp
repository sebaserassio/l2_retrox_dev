...
{
	ifstream file(g_ChangeFolders.augmentation_skill_txt.c_str());
	if(file)
	{
		mSkills.clear();
		string sLine;
...
void CAugmentation::ReadStatData()
{
	...
	double Value;
	int nAugmentID;
	ifstream file(g_ChangeFolders.augmentation_stat_txt.c_str());
	if(file)
	{
		mStats.clear();
		while(file)
...
{
	wstring str = ReadFileW(g_ChangeFolders.augmentation_name_txt);	//wstring filew = ReadFileW(L"..\\Script\\augmentation_name.txt");
	wstringstream sstr;
	sstr << str;
	wstring wLine;
	int Counter = 0;
...
UINT CAugmentation::GenerateAugmentation(int nLifeStoneGrade, int nLifeStoneLevel, int nItemID)
{
	...
	if(pUser->ValidUser())
	{
		...
		if(pWeapon && pLifeStone->IsValidItem() && pGemStone->IsValidItem())
		{
			...
			if(pWeapon->nAugmentationID == 0 && pLifeStone->IsLifeStone())
			{
				...
				{
					UINT nAugmentID = GenerateAugmentation(nLifeStoneGrade, nLifeStoneLevel, pLifeStone->pSID->nItemID);
