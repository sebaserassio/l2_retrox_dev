...
int CAccountDB::GetCustomSkin(const int char_id)
{ 
	std::map<int, int>::const_iterator i(CustomSkins.find(char_id));
	if (i == CustomSkins.end()) {
		return -1;
	} else {
		return i->second;
	}
...
void CAccountDB::AddCustomSkin2(int char_id, int skin)
{ 
	CustomSkins.insert ( std::pair<int,int>(char_id,skin) );
}

void CAccountDB::AddCustomSkin(int char_id, int skin)
{ 
	std::map<int, int>::iterator i(CustomSkins.find(char_id));
	if (i == CustomSkins.end()) {
		CustomSkins.insert ( std::pair<int,int>(char_id,skin) );
	} else {
		i->second = skin;
...
int CAccountDB::GetCustomSex(const int char_id)
{ 
	std::map<int, int>::const_iterator i(CustomSex.find(char_id));
	if (i == CustomSex.end()) {
		return -1;
	} else {
		return i->second;
...
void CAccountDB::AddCustomSex(int char_id, int sex)
{ 
	std::map<int, int>::iterator i(CustomSex.find(char_id));
	if (i == CustomSex.end()) {
		CustomSex.insert ( std::pair<int,int>(char_id,sex) );
	} else {
		i->second = sex;
	}
}

void CAccountDB::AddCustomSex2(int char_id, int sex)
{ 
		CustomSex.insert ( std::pair<int,int>(char_id,sex) );
}