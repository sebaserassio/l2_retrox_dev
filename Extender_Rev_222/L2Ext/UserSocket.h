...
struct OfflineShopRecord; /* LEWIS-OSR */
struct OfflineShopBuffRecord; /* LEWIS-OSR */

struct SocketExtData
{
	/* 02 */ UINT IsLoadOfflineShop;
	...
	/* xx */ BYTE md5Checksum_i[32];
	/* xx */ bool is_osr; /* LEWIS-OSR */
	/* xx */ bool is_osr_buff; /* LEWIS-OSR */
	/* xx */ OfflineShopRecord *osr;
	/* xx */ //OfflineShopBuffRecord *osr_buff;
	/* xx */ wstring RealIP;
	/* xx */ UINT m_BotStringNumber1;
	/* xx */ UINT m_BotStringNumber2;
	/* xx */ UINT m_BotStringNumber3;
	/* xx */ UINT m_BotStringNumber4;
	/* xx */ CIPAddress IPaddr;
	/* xx */ bool FileCorrupt;
	/* xx */ int TrheadProccesAdrenaline;
	/* xx */ DWORD packetTimestamp[256];
			 UINT FileCorruptCounter;
			 UINT FileCorruptCounterError;
			 UINT TrheadProccesAdrenalineCounterError;
			 WCHAR hwidString[255];
			 BYTE opcodeTableOut[256];

	PINData* lpPD;
	UINT selectedCharId;
	SocketExtData() : lpPD(0), selectedCharId(0), IsLoadOfflineShop(0)
	{
		memset(md5Checksum, 0, sizeof(md5Checksum));
		memset(md5Checksum_i, 0, sizeof(md5Checksum_i));
		for(UINT n=0;n<7;n++)
		{
			baseClass[n] = UINT_MAX;
		}
		
		is_osr = false; /* LEWIS-OSR */
		is_osr_buff = false; /* LEWIS-OSR */

		m_BotStringNumber1 = 0;
		m_BotStringNumber2 = 0;
		m_BotStringNumber3 = 0;
		m_BotStringNumber4 = 0;
...
public:
	...
	/* 0160 */ unsigned char padding0x0160[0x0168 - 0x0160];
	/* 0168 */ int m_nLeftTime;
	/* 016C */ unsigned char padding0x016C[0x019C - 0x016C];
	...
	/* 01FC */ UINT _nUnkn1FC[129];				//_nUnkn1FC[130] 
			   int m_nSocketCharacterTimeStamp;
	...
	/* 0450 */ int accountId;
	/* 0454 */ int _dw0454;
	/* 0458 */ int m_nPlayTime_china;
	/* 045C */ int m_nStartPlayTimeStamp_china;
	/* 0460 */ int _dw0460;
	/* 0464 */ int _dw0464;
	/* 0468 */ int _dw0468;
	/* 046C */ int _dw046C;
