...
int CInventory::GetAdenaCustomAmount(CInventory* Inventory)
{
	INT32 itemCount = 0;

	User *pUser = Inventory->GetOwner()->GetUser();

	if (pUser->ValidUser())
	{
		
		CItem *pItem = pUser->inventory.GetFirstItemByClassID(g_PrivateStore_ItemId);
...
bool CInventory::ReadLock(int nItemIndex)
{
	typedef bool(*f)(CInventory*, int);
	return f(0x6119D0L)(this, nItemIndex);
}
