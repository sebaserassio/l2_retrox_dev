...
void CChampionship::LoadINI()
{
	...
	if(GetPrivateProfileString(section, _T("BlueTeamLocation"), 0, sTemp, 0x8000, g_ConfigFile))
	{
		...
		while(sstr >> pos)
		{
			...
			pos = Utils::ReplaceChar(pos, _T("/"), _T(" "));
			tstringstream sstr;
			sstr << pos;
...
void CChampionship::AddTeam(UINT id, std::wstring name, UINT charId1, UINT charId2, UINT charId3, INT32 points, INT32 winCount, INT32 loseCount, INT32 drawCount, INT32 killCount, INT32 deathCount, UINT teamCount)
{
	...
	if(m_enabled)
	{
		...
		if(size == teamCount)
		{
			g_Log.Add(CLog::Blue, "[%s] Loaded [%d] teams", __FUNCTION__, size);
			g_Log.Add(CLog::Blue, "[%s] Loading matches...", __FUNCTION__);
...
void CChampionship::AddMatch(UINT id, UINT state, UINT startTime, UINT teamId1, UINT teamId2, UINT winnerId, UINT matchCount)
{
	...
	if(m_enabled)
	{
		...
		{
			g_Log.Add(CLog::Error, "[%s] Cannot find team data for teamId[%d] matchId[%d]!", __FUNCTION__, teamId1, id);
			pData->teamData[0] = 0;
		}
		...
		{
			g_Log.Add(CLog::Error, "[%s] Cannot find team data for teamId[%d] matchId[%d]!", __FUNCTION__, teamId2, id);
			pData->teamData[1] = 0;
		}
		...
		if(size == matchCount)
		{
			g_Log.Add(CLog::Blue, "[%s] Loaded [%d] matches", __FUNCTION__, size);
			m_loadedDB = TRUE;
		}
	}
	unguard;
...
void CChampionship::OnLeaveWorld(User *pUser)
{
	...
	{
		if(pUser->pED->championshipUser.matchId > 0)
		{
			//remove user from match
			Lock();
			if(m_pMatch)
			{
				if(m_pMatch->id == pUser->pED->championshipUser.matchId)
				{
					map<UINT, User*>::iterator userIter = m_pMatch->m_users.find(pUser->nDBID);
		...
		if(m_pMatch)
		{
			if(m_pMatch->state == Championship::MatchTeleporting)
			{
				for(int n=0;n<2;n++)
				{
					...
					for(int m=0;m < 3; m++)
					{
						if(pUser->nDBID == m_pMatch->teamData[n]->charId[m])
						{
							if(User *pUserEx = m_pMatch->AddUser(pUser->nDBID, m_pMatch->teamData[n]->type))
							{
			...
			case Championship::MatchTeleporting:
				{
					...
					}else
					{
						...
						for(map<UINT, User*>::iterator userIter = m_pMatch->m_users.begin(); userIter != m_pMatch->m_users.end(); userIter++)
						{
							...
							if(pUser->ValidUser())
							{
								if(pUser->pED->championshipUser.teamType == Championship::TeamBlue)
									blueCount++;
								else if(pUser->pED->championshipUser.teamType == Championship::TeamRed)
									redCount++;
							}
						}
						...
						if(blueCount == 3 && redCount == blueCount)
						{
							...
							for(map<UINT, User*>::iterator userIter = m_pMatch->m_users.begin(); userIter != m_pMatch->m_users.end(); userIter++)
							{
								...
								if(pUser->ValidUser())
								{
									int teamIdx = pUser->pED->championshipUser.teamType -1;
									if(teamIdx >= 0 && teamIdx < 2)
									{
										...
										pUser->pED->championshipUser.state = Championship::UserPreparing;
										ValidateItems(pUser);
						//				pUser->Healthy();
										...
										if(teleportIdx[teamIdx] < 3)
										{
											...
											{
												pUser->pED->championshipUser.orgPos = pUser->pSD->Pos;
												//3032	1	a,You are now being moved to the Championship Stadium.\0	0	79	9B	B0	FF	a,	a,	0	0	0	0	0	a,	a,none\0
												pUser->pSocket->SendSystemMessage(3032);
												pUser->TeleportToLocation(m_teleportPos[teamIdx][teleportIdx[teamIdx]].x, m_teleportPos[teamIdx][teleportIdx[teamIdx]].y, m_teleportPos[teamIdx][teleportIdx[teamIdx]].z, true);
											}
											teleportIdx[teamIdx]++;
										}
									}
			...
			case Championship::MatchPreparing:
				{
					...
					}else
					{
						...
						for(map<UINT, User*>::iterator userIter = m_pMatch->m_users.begin();userIter!=m_pMatch->m_users.end();userIter++)
						{
							...
							if(pUser->ValidUser())
							{
								pUser->pED->championshipUser.state = Championship::UserFighting;
								pUser->SendUserInfo();
								pUser->SendCharInfo();
							}
						}
			...
			case Championship::MatchTeleportBack:
				{
					...
					}else
					{
						...
						for(map<UINT, User*>::iterator userIter = m_pMatch->m_users.begin();userIter!=m_pMatch->m_users.end();userIter++)
						{
							...
							if(pUser->ValidUser())
							{
								if(pUser->pED->championshipUser.orgPos.x != 0 || pUser->pED->championshipUser.orgPos.y != 0)
								{
								//	pUser->Healthy();
										//Heal up the user
									pUser->pSD->fCP = pUser->pSD->MaxCP;
									...
									pUser->TeleportToLocation(pUser->pED->championshipUser.orgPos.x, pUser->pED->championshipUser.orgPos.y, pUser->pED->championshipUser.orgPos.z, true);
								}
							}
						}
					}
			...
			if(m_loadedDB)
			{
				if(m_autoReloadDB)
				{
					if( (m_reloadDbTime+m_autoReloadDB) < time(NULL))
					{
						...
						g_DB.RequestLoadChampionshipMatches();
						m_reloadDbTime = time(NULL);
						g_Log.Add(CLog::Blue, "[%s] Reloading matches...", __FUNCTION__);
					}
				}
...
void CChampionship::OnDie(User *pUser)
{
	...
	if(m_enabled)
	{
		...
		{
			Lock();
			if(m_pMatch && pUser->pED->championshipUser.matchId == m_pMatch->id)
			{
				if(pUser->pED->championshipUser.state = Championship::UserFighting)
				{
					if(pUser->pED->championshipUser.teamType == Championship::TeamBlue)
					{
	...
	if(pUser->ValidUser())
	{
		for(int n=0;n<29;n++)
		{
			...
			if(index > 0)
			{
				...
				if(pItem)
				{
					if(IsBlockedItem(pItem->pSID->nItemID))
					{
				//		pUser->UnEquipItem(pItem);
						continue;
					}
					if(IsBlockedCrystalType(pItem->pII->nCrystalType))
					{
//						pUser->UnEquipItem(pItem);
						continue;
					}
	...
	//				}
					...
					if(pItem->GetWeapon())
					{
						...
						{
							if( pItem->pSID->nEnchantLevel > (m_maxWeaponEnchant + 1) )
							{
						//		pUser->UnEquipItem(pItem);
								continue;
							}
						...
						{
							if(pItem->pSID->nEnchantLevel > m_maxWeaponEnchant)
							{
						//		pUser->UnEquipItem(pItem);
								continue;
							}
...
//		return false;
	...
	if(pItem->GetWeapon() && m_maxWeaponEnchant > 0)
	{
		if(pItem->pII->weaponType == WeaponDual)
		{
			if( pItem->pSID->nEnchantLevel > (m_maxWeaponEnchant + 1) )
