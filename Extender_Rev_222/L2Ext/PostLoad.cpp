...
CDBSocket *g_AuthSocket = NULL;

using namespace License;



char UltimoRestart[80];
...
void CPostLoad::InitKey(LPVOID pInstance, CLog::LogType Type, wchar_t* Msg)
{
	...
	//{
		...
		g_HookManager.WriteCall( 0x51B4E1, CPostLoad::OnAuthConstruct);

		g_HookManager.WriteCall( 0x422B5B, ExitProcess);
	//}
	g_Log.Add(Type, Msg);
	VIRTUALIZER_TIGER_RED_END;
}
...
void CPostLoad::FirstLoad(LPVOID Instance)
{
	...
	//{
		...
		//{
			...
			g_OfflineShopRestore.Initialize();
...
void CPostLoad::ThirdLoad(LPVOID pInstance)
{
	...
	//{
		...
		g_PKPvPStatus.Init();
		g_EventStats.Init();
		g_OlympiadStatus.Init();
		g_EpicItems.Init();
		g_ClanPvPStatus.Init();
		g_ClanReputationRank.Init();
		...
		g_DB.RequestLoadSkin();
		g_DB.RequestLoadSex();
		g_SchemeBuffer.Init();

		NuevasFunciones::Init();
		...
		//g_Log.Add(CLog::Error, "[%S] here!!!!", __FUNCTION__);
			...
			strftime(UltimoRestart, sizeof(UltimoRestart), "%d/%m/%Y %H:%M", &timeinfo);
...
CDBSocket * CPostLoad::OnAuthConstruct(CDBSocket *pSocket, SOCKET s, void *packettable)
{
	g_AuthSocket = pSocket;
	typedef CDBSocket *(*f)(CDBSocket*,SOCKET,void*);
	CDBSocket *ret = f(0x54A1F0)(pSocket,s,packettable);

	//Fix for 2k12
	g_Win2k12.SetTimeStartedDone(true);

	g_OfflineShopRestore.OnServerLoaded();
