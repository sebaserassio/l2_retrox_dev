...
{
	wstring wFile = ReadFileW(g_ChangeFolders.SkillData_txt);
	wstringstream sstr;
	sstr << wFile;
	int nLine = 0;
	int nSkills = 0;
...
void SkillDataParser::InitMaps()
{
	...
	m_SkillEffect[(wstring)L"p_pvp_physical_skill_dmg_bonus_by_weapon"] = P_PVP_PHYSICAL_SKILL_DMG_BONUS_BY_WEAPON;
	...
	m_SkillEffect[(wstring)L"i_spawn_aganthion"] = I_SPAWN_AGANTHION;
	...
	m_SkillEffect[(wstring)L"i_dispel_by_category_rand"] = I_DISPEL_BY_CATEGORY_RAND;
	m_SkillEffect[(wstring)L"i_dispel_by_category_rand50"] = I_DISPEL_BY_CATEGORY_RAND_50;
	...
	m_SkillEffect[(wstring)L"p_defence_attribute_by_class"] = P_DEFENCE_ATTRIBUTE_BY_CLASS;
	...
	m_SkillEffect[(wstring)L"p_frenzy_activo"] = P_FRENZY_ACTIVO;

	m_SkillEffect[(wstring)L"p_stop_exp"] = P_STOP_EXP;
	m_SkillEffect[(wstring)L"p_global_shout"] = P_GLOBAL_SHOUT;
	...
	m_SkillEffect[(wstring)L"i_give_item_maxcount"] = I_GIVE_ITEM_MAX_COUNT;
	m_SkillEffect[(wstring)L"i_acquire_skill"] = I_ACQUIRE_SKILL;

	m_SkillEffect[(wstring)L"i_dispel_skill_id"] = I_DISPEL_SKILL_ID;
	...
	m_SkillEffect[L"i_report_bot"] = I_REPORT_BOT;

	m_SkillEffect[L"i_cancel_min_max"] = I_CANCEL_MIN_MAX;
	m_SkillEffect[L"p_custom_conversion"] = P_CUSTOM_CONVERSION;
	m_SkillEffect[L"i_cancel_min_max_backup"] = I_CANCEL_MIN_MAX_BACKUP;
	m_SkillEffect[L"i_sweep_consume_body"] = I_SWEEP_CONSUME_BODY;
	m_SkillEffect[L"i_cancel_slot_chance_backup"] = I_CANCEL_SLOT_CHANCE_BACKUP;

	
	m_SkillEffect[L"p_unequip_item"] = P_UNEQUIP_ITEM;
	...
	m_OperateCond[L"op_freeslots"] = OP_HAS_FREE_SLOTS;

	m_OperateCond[(wstring)L"op_remain_hp_diff"] = OP_REMAIN_HP_DIFF;
	...
	m_IntParam[L"mass_sweep"] = IP_MASS_SWEEP;
...
int SkillDataParser::ChangeWordID(LPVOID Param1)
{
	...
	if((*(LPDWORD)Param1) == 0xAE2538)
	{
		...
		if(nEffectID == P_DEFENCE_ATTRIBUTE_BY_CLASS)
		{
			//change format attribute_name -> diff
			if(nID == 65831)
				nID = 65829;
	...
	if(bChangeID)
	{
		...
		case P_PVP_PHYSICAL_SKILL_DMG_BONUS_BY_WEAPON:
			{
				bChangeID = false;
			//	g_Log.Add(CLog::Blue, "[SkillDataParser] Fixing ID[%d] to [%d]" , nID, PUMP_EFFECT_BY_WEAPON);
				return PUMP_EFFECT_BY_WEAPON;
		...
		case I_CANCEL_MIN_MAX:
			{
				bChangeID = false;
				return INSTANT_EFFECT_TWO_ARG;
				break;
			}
		case P_CUSTOM_CONVERSION:
			{
				bChangeID = false;
				return PUMP_EFFECT_ONE_ARG;
				break;
		...
		case I_CANCEL_MIN_MAX_BACKUP:
			{
				bChangeID = false;
				return INSTANT_EFFECT_TWO_ARG;
				break;
			}
		case I_CANCEL_SLOT_CHANCE_BACKUP:
			{
				bChangeID = false;
				return INSTANT_EFFECT_TWO_ARG;
				break;
			}
		case I_DISPEL_BY_CATEGORY_RAND:
			{
				bChangeID = false;
				return INSTANT_EFFECT_TWO_ARG;
				break;
			}
		case I_DISPEL_BY_CATEGORY_RAND_50:
			{
				bChangeID = false;
				return INSTANT_EFFECT_TWO_ARG;
				break;
		...
		case P_DEFENCE_ATTRIBUTE_BY_CLASS:
			{
				bChangeID = false;
				return PUMP_EFFECT_TWO_ARGS;
				break;
...
LPVOID SkillDataParser::CreatePumpEffectNoArg(int nSize)
{
	...
	case P_FRENZY_ACTIVO:
		{
			CSkillEffect_p_frenzy_activo *pEffect = new CSkillEffect_p_frenzy_activo;
			nEffectID = 0;
			return pEffect;
			break;
		}
	case P_STOP_EXP:
		{
			CSkillEffect_p_stop_exp *pEffect = new CSkillEffect_p_stop_exp;
			nEffectID = 0;
			return pEffect;
			break;
		}
	case P_GLOBAL_SHOUT:
		{
			CSkillEffect_p_global_shout *pEffect = new CSkillEffect_p_global_shout;
			nEffectID = 0;
			return pEffect;
			break;
		}
...
LPVOID SkillDataParser::CreatePumpEffectOneArg(int nSize)
{
	...
	case P_CUSTOM_CONVERSION:
		{
			CSkillEffect_p_custom_conversion* pEffect = new CSkillEffect_p_custom_conversion;
			nEffectID = 0;
			return pEffect;
			break;
		}
	case P_UNEQUIP_ITEM:
		{
			CSkillEffect_p_unequip_item* pEffect = new CSkillEffect_p_unequip_item;
			nEffectID = 0;
			return pEffect;
			break;
		}
...
LPVOID SkillDataParser::CreatePumpEffectTwoArgs(int nSize)
{
	...
	case P_DEFENCE_ATTRIBUTE_BY_CLASS:
		{
			CSkillEffect_p_defence_attribute_by_class *pEffect = new CSkillEffect_p_defence_attribute_by_class;
			nEffectID = 0;
			return pEffect;
			break;
		}
...
LPVOID SkillDataParser::CreatePumpEffectByWeapon(int nSize)
{
	...
	case P_PVP_PHYSICAL_SKILL_DMG_BONUS_BY_WEAPON:
		{
			CSkillEffect_p_pvp_physical_skill_dmg_bonus_by_weapon *pEffect = new CSkillEffect_p_pvp_physical_skill_dmg_bonus_by_weapon;
		//	g_Log.Add(CLog::Blue, "[SkillDataParser] Creating p_critical_damage_by_weapon effect");
			nEffectID = 0;
			return pEffect;
			break;
...
LPVOID SkillDataParser::CreateInstantEffect(int nSize)
{
	...
	case I_SWEEP_CONSUME_BODY:
		{
			CSkillEffect_i_sweep_consume_body *pEffect = new CSkillEffect_i_sweep_consume_body;
			nEffectID = 0;
			return pEffect;
		}
	case I_REPORT_BOT:
		{
			CSkillEffect_i_report_bot *pEffect = new CSkillEffect_i_report_bot;
			nEffectID = 0;
			return pEffect;
		}
	default:
...
LPVOID SkillDataParser::CreateInstantEffectTwoArg(int nSize)
{
	...
	case I_SPAWN_AGANTHION:
		{
			CSkillEffect_i_spawn_aganthion *pEffect = new CSkillEffect_i_spawn_aganthion;
			nEffectID = 0;
			return pEffect;
			break;
		}
	...
	case I_CANCEL_MIN_MAX:
		{
			CSkillEffect_i_cancel_min_max *pEffect = new CSkillEffect_i_cancel_min_max;
			nEffectID = 0;
			return pEffect;
			break;
		}
	case I_CANCEL_MIN_MAX_BACKUP:
		{
			CSkillEffect_i_cancel_min_max_backup *pEffect = new CSkillEffect_i_cancel_min_max_backup;
			nEffectID = 0;
			return pEffect;
			break;
		}
	case I_CANCEL_SLOT_CHANCE_BACKUP:
		{
			CSkillEffect_i_dispel_by_slot_probability_backup *pEffect = new CSkillEffect_i_dispel_by_slot_probability_backup;
			nEffectID = 0;
			return pEffect;
			break;
		}
	case I_DISPEL_BY_CATEGORY_RAND:
		{
			CSkillEffect_i_dispel_by_category_rand *pEffect = new CSkillEffect_i_dispel_by_category_rand;
			nEffectID = 0;
			return pEffect;
			break;
		}
	case I_DISPEL_BY_CATEGORY_RAND_50:
		{
			CSkillEffect_i_dispel_by_category_rand50 *pEffect = new CSkillEffect_i_dispel_by_category_rand50;
			nEffectID = 0;
			return pEffect;
			break;
		}
	...
	case I_GIVE_ITEM_MAX_COUNT:
		{
			CSkillEffect_i_give_item_maxcount *pEffect = new CSkillEffect_i_give_item_maxcount;
			nEffectID = 0;
			return pEffect;
			break;
		}
	...
	case I_DISPEL_SKILL_ID:
		{
			CSkillEffect_i_dispel_skill_id *pEffect = new CSkillEffect_i_dispel_skill_id;
			nEffectID = 0;
			return pEffect;
			break;
		}
...
LPVOID SkillDataParser::CreateOperateConditionOneVar(int nSize)
{
	...
	case OP_REMAIN_HP_DIFF:
		{
			CSkillOperateCondition_op_remain_hp_diff *pCond = new CSkillOperateCondition_op_remain_hp_diff;
			nEffectID = 0;
			return pCond;
			break;
		}
