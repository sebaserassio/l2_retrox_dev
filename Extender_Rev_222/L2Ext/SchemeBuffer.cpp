...
void CSchemeBuffer::LoadData()
{
	...
	wstringstream file(ReadFileW(g_ChangeFolders.SchemeBuffer_txt));
	wstring line;
	BOOL enabled = FALSE;
	m_Buffs.clear();
	m_RemoteBuffs.clear();
	while(getline(file, line))
	{
		...
		if(line.find(L"setting_begin") == 0)
		{
			...
			m_AllowEventFight = Parser::ParseOptionInt(line, L"allow_eventfight");
			m_PeaceZoneOnly = Parser::ParseOptionInt(line, L"peacezone_only");
			m_PeaceZoneOnlySchemeR = Parser::ParseOptionInt(line, L"peacezone_only_scheme_remote");
			m_freeLevel = Parser::ParseOptionInt(line, L"freeLevel", 10);
		...
		}else if(line.find(L"buff_begin") == 0)
		{
			...
			int id = Parser::ParseOptionInt(line, L"id", 0);
			if (id > 0)
			{
				INT32 skillId = 0, level = 0;
				g_SkillDB.GetSkillIdLevelByName(Parser::ParseOptionString(line, L"name").c_str(), &skillId, &level);
				if(skillId > 0)
				{
					SkillPchId skill(skillId, level);
					buff.skill.pchId = skill.pchId;
					buff.id = id;
				}
			}
			else
		...
		}else if(line.find(L"remote_buff_begin") == 0)
		{
			...
			buff.maxLevel = Parser::ParseOptionInt(line, L"max_level");
			buff.abnormalTime = Parser::ParseOptionInt(line, L"abnormal_time");
			buff.abnormalTime2 = Parser::ParseOptionInt(line, L"abnormal_time2");
			buff.requiredItem.itemId = Parser::ParseOptionInt(line, L"required_item_id");
			buff.requiredItem.amount = Parser::ParseOptionInt(line, L"required_amount");
			if(CSkillInfo *pSI = g_SkillDB.GetSkillInfo(buff.skill.skillId, buff.skill.level))
			{
		...
		}else if(line.find(L"macro_begin") == 0)
		{
			...
			{
				INT32 skillId = 0, level = 0;
				g_SkillDB.GetSkillIdLevelByName(pchId.c_str(), &skillId, &level);
				if(skillId > 0)
				{
					//SkillPchId pch(pchId);
					if(CSkillInfo *pSI = g_SkillDB.GetSkillInfo(skillId, level))
					{
...
bool CSchemeBuffer::CanUse(User *pUser)
{
	...
	if( (pUser->pED->schemeBufferUser.useTick < currentTick) || ( (pUser->pED->schemeBufferUser.useTick - m_UseDelay) > currentTick) )
	{
		...
		if(pUser->pED->tvtUser.status == TvT::UserFighting && !m_AllowEventFight)
		{
			pUser->SendSystemMessage(L"You cannot perform this action during Event fight!");
			unguard;
			return false;
		}
		unguard;
...
SchemeBuff& CSchemeBuffer::Get(UINT buffId, bool remote)
{
	...
	if(remote)
	{
		...
		{
			if(m_RemoteBuffs[n].skill.pchId == buffId || m_RemoteBuffs[n].id == buffId)
			{
				unguard;
	...
	}else
	{
		...
		{
			if(m_Buffs[n].skill.pchId == buffId || m_Buffs[n].id == buffId)
			{
				unguard;
				return m_Buffs[n];
			}
...
bool CSchemeBuffer::RequestBuff(User *pUser, UINT buffId, bool summon, bool remote, int nbufftime)
{
	guard;
	if(m_Enabled && CanUse(pUser))
	{
		...
		if(buff.pSI)
		{
			...
			if(buff.price.itemId > 0 && buff.price.amount > 0 && pUser->pSD->nLevel > (int)m_freeLevel)
			{
			...
			if(nbufftime)
			{
				//pUser->QueueBuff(buff.pSI, summon, buff.abnormalTime2, pUser->talkTargetId);
				pUser->QueueBuff(buff.pSI, summon, buff.abnormalTime2);
			}
			else
			{
				pUser->QueueBuff(buff.pSI, summon, buff.abnormalTime);
				//pUser->QueueBuff(buff.pSI, summon, buff.abnormalTime, pUser->talkTargetId);
			}
			unguard;
			return true;
		}else
...
bool CSchemeBuffer::RequestAddToList(User *pUser, UINT buffId, bool summon, bool remote, int Id)
{
	guard;
	if(m_Enabled /*&& CanUse(pUser)*/)
	{
...
void CSchemeBuffer::ValidateBuffList(User *pUser, bool summon, bool remote, int Id)
{
	guard;
	//check for min/max level in existing buff list
	vector<SkillPchId> buffs;
	pUser->pED->schemeBufferUser.lock.Enter();
	vector<SkillPchId>& buffList = pUser->pED->schemeBufferUser.GetList(summon, remote, Id);
...
bool CSchemeBuffer::RequestUseList(User *pUser, bool summon, bool remote, int abuse, int Id)
{
	guard;
	if(m_Enabled && CanUse(pUser))
	{
		...
		{
			SchemeBufferUser& sbu = pUser->pED->schemeBufferUser;
			if((Id ==1 && sbu.buffList1.size() > 0) || (Id ==2 && sbu.buffList2.size() > 0) || (Id ==3 && sbu.buffList3.size() > 0) || (Id ==4 && sbu.buffList4.size() > 0) || (Id ==5 && sbu.buffList5.size() > 0) || (Id ==6 && sbu.buffList6.size() > 0) || (Id ==7 && sbu.buffList7.size() > 0) || (Id ==8 && sbu.buffList8.size() > 0) || (Id ==9 && sbu.buffList9.size() > 0) )
			{
				...
				{
					pUser->SendSystemMessage(L"You don't have enough items to use buff list.");
					unguard;
					return false;
				}
			}else
		...
		if(remote && !pUser->inPeaceZone && m_PeaceZoneOnlySchemeR)
		{
			pUser->SendSystemMessageRed(L"You cannot perform this action outside a peace zone!");
			unguard;
			return false;
		}
		pUser->QueueBuffList(summon, remote, abuse, Id);
	}
	unguard;
	return false;
}

bool CSchemeBuffer::RequestClearList(User *pUser, bool summon, bool remote, int Id)
{
	guard;
	if(m_Enabled && CanUse(pUser))
	{
		SchemeBufferUser& sbu = pUser->pED->schemeBufferUser;
		sbu.lock.Enter();
		vector<SkillPchId>& list = sbu.GetList(summon, remote, Id);
		if(list.size() > 0)
		{
			list.clear();
		}
