...
// Cuenta y devuelve en vUsers los miembros dentro del AABB [minPos, maxPos]
UINT CParty::GetAllMembersInZone(UserVector& vUsers, const FVector& minPos, const FVector& maxPos, bool append /*=false*/)
{
	guard;

	if (!append) vUsers.clear();
	// Evitar realocaciones si vas a pushear varios
	vUsers.reserve(vUsers.size() + memberCount);

	// Normaliza por si vienen invertidos
	const double minX = std::min<double>(minPos.x, maxPos.x);
	const double maxX = std::max<double>(minPos.x, maxPos.x);
	const double minY = std::min<double>(minPos.y, maxPos.y);
	const double maxY = std::max<double>(minPos.y, maxPos.y);
	const double minZ = std::min<double>(minPos.z, maxPos.z);
	const double maxZ = std::max<double>(minPos.z, maxPos.z);
		...
		for (UINT n = 0; n < memberCount; ++n)
		{
			if (User* pUser = User::GetUserBySID(&memberId[n]))
			{
				if (pUser->pSD->nActiveUser)
				{
					if(pUser->pSD->nAlive)
					{
						const double x = pUser->pSD->Pos.x;
						const double y = pUser->pSD->Pos.y;
						const double z = pUser->pSD->Pos.z;

						// Inclusivo en bordes; cambia <= por < si querés excluirlos
						if (x >= minX && x <= maxX &&
							y >= minY && y <= maxY &&
							z >= minZ && z <= maxZ)
						{
