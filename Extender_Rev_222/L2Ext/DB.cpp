...
void CDB::RequestSaveVipChatId(int charId, int vipChatId)
{
	if(charId)
	{
		Send("chdd", 0xF9, CI_REQUEST_SAVE_VIP_CHAT_ID, charId, vipChatId);
	}
}
...
void CDB::RequestLoadSemanaStatus()
{
	Send("ch", 0xF9, CI_REQUEST_LOAD_SEMANA_STATUS);
}
...
void CDBPacket::Init()
{
	...
	Bind( CO_REPLY_LOAD_SEMANA_STATUS, CDBPacket::ReplyLoadSemanaStatus);
	...
	Bind( CO_REPLY_CI_REQUEST_LOAD_HERO_TIMESTAMP_EX, CDBPacket::ReplyLoadHeroTime);
...
bool CDBPacket::ReplyLoadPKStatus(CDBSocket *pSocket, const unsigned char* packet, UINT packetId)
{
	...
	WCHAR wClanName[25] = { 0 };
	WCHAR wAllyName[25] = { 0 };
	INT32 crestClan = 0;
	INT32 crestAlly = 0;

	Disassemble(packet, "dSdddSdSd", &charId, sizeof(wName), wName, &classId, &level, &pkAmount, sizeof(wClanName), wClanName, &crestClan, sizeof(wAllyName), wAllyName, &crestAlly);
	if(charId > 0)
	{
		g_PKPvPStatus.OnDBLoadPK(charId, wName, classId, level, 0, pkAmount, wClanName, wAllyName, crestClan, crestAlly);
	}
...
bool CDBPacket::ReplyLoadPvPStatus(CDBSocket *pSocket, const unsigned char* packet, UINT packetId)
{
	...
	Disassemble(packet, "dSdddSdSd", &charId, sizeof(wName), wName, &classId, &level, &pvpAmount, sizeof(wClanName), wClanName, &crestClan, sizeof(wAllyName), wAllyName, &crestAlly);
	if(charId > 0)
	{
		g_PKPvPStatus.OnDBLoadPvP(charId, wName, classId, level, pvpAmount, 0, wClanName, wAllyName, crestClan, crestAlly);
	}
...
bool CDBPacket::ReplyLoadDailyStatus(CDBSocket *pSocket, const unsigned char* packet, UINT packetId)
{
	...
	if(charId > 0)
	{
		g_PKPvPStatus.OnDBLoadDaily(charId, wName, classId, level, pvpAmount, 0, wClanName, wAllyName, crestClan, crestAlly);
	}
...
bool CDBPacket::ReplyLoadSemanaStatus(CDBSocket *pSocket, const unsigned char* packet, UINT packetId)
{
	guard;
	UINT charId = 0, classId = 0;
	INT32 level = 0, pvpAmount = 0;
	...
	if(charId > 0)
	{
		g_PKPvPStatus.OnDBLoadSemana(charId, wName, classId, level, pvpAmount, 0, wClanName, wAllyName, crestClan, crestAlly);
	}
...
{
	guard;
	int vipLevel = 0, charId = 0, vipchatId = 0;
	DWORD vipTimestamp = 0;
	Disassemble(packet, "dddd", &charId, &vipLevel, &vipTimestamp, &vipchatId);
	if(charId)
	{
		User *pUser = g_UserDB.GetUserByDBID(charId);
		...
		if(pUser->ValidUser())
		{
			...
			pUser->pSD->vipchatId = vipchatId;
			g_VIPSystem.OnLoad(pUser);
			g_AccountDB.SetVIPLevel(pUser->pSocket->accountId, vipLevel, vipTimestamp);
		}else
		{
			g_Log.Add(CLog::Error, "[%s] Invalid user[%d][%d].", __FUNCTION__, charId, pUser);
		}
	}
	unguard;
	return false;
...
void CDB::RequestSaveHeroTime(UINT nDBID, UINT timestamp)
{
	if(nDBID)
	{
		Send("chdd", 0xF9, CI_REQUEST_SAVE_HERO_TIMESTAMP, nDBID, timestamp);
	}
}

void CDB::RequestLoadHeroTime(UINT nDBID)
{
	if(nDBID)
	{
		Send("chd", 0xF9, CI_REQUEST_LOAD_HERO_TIMESTAMP, nDBID);
	}
}
...
bool CDBPacket::ReplyLoadHeroTime(CDBSocket *pSocket, const unsigned char* packet, UINT packetId)
{
	guard;
	UINT charId;
	UINT timeStamp;
