...
bool CInfinityShot::AutoOn(User *pUser, int itemId)
{
	...
	if(CItem *pItem = pUser->inventory.GetFirstItemByClassID(itemId))
	{
		...
		{
			if(pItem->pII->etcType == EtcItemInfinitySoulshot || pItem->pSID->nItemID == 1467)
			{
				//check for olympiad
...
//								pUser->SendSystemMessage(&msg);
							...
							int consume = 1;
							if(pUser->inventory.HaveItemByClassID(pItem->pSID->nItemID, pWI->soulshotCount))
								consume = pWI->soulshotCount;

								g_DB.RequestUseItem(pItem,pUser,consume);
								unguard;
								return true;
							}
						}else
...
//								pUser->SendSystemMessage(&msg);
							...
							int consume = 1;
							if(pUser->inventory.HaveItemByClassID(pItem->pSID->nItemID, pWI->spiritshotCount))
								consume = pWI->spiritshotCount;
...
/**/
bool CInfinityShot::AutoOn_Normal(User *pUser, int itemId)
{
	guard;

	if(CItem *pItem = pUser->inventory.GetFirstItemByClassID(itemId))
	{
		UINT itemId = pItem->pSID->nItemID;
		//UINT etcType = pItem->pII->etcType;
		if( (itemId > 1462 && itemId < 1468) || itemId == 5789 || itemId == 1835 || (itemId > 2508 && itemId < 2515) || itemId == 5790 || (itemId > 3946 && itemId < 3953) )
		{
			if( (itemId > 1462 && itemId < 1468) || itemId == 5789 || itemId == 1835 )
			{
				...
				if(m_AllowSoulShotInOlympiad || !pUser->olympiadUser.IsInside())
				{
					if(pUser->pED->autoInfinitySoulshot_2[pItem->pII->nCrystalType])
					{
						AutoOff_Normal(pUser, pUser->pED->autoInfinitySoulshot_2[pItem->pII->nCrystalType]);
					}
					pUser->pED->autoInfinitySoulshot_2[pItem->pII->nCrystalType] = itemId;
				}else
				{
					//1508	1	a,You cannot use that item in a Grand Olympiad Games match.\0	0	79	9B	B0	FF	a,ItemSound3.sys_shortage\0	a,	0	0	0	0	0	a,	a,none\0
					CSystemMessage msg(1508);
			...
			}else if( (itemId > 2508 && itemId < 2515) || itemId == 5790 )
			{
				if(m_AllowSpiritShotInOlympiad || !pUser->olympiadUser.IsInside())
				{
					if(pUser->pED->autoInfinitySpiritshot_2[pItem->pII->nCrystalType])
					{
						AutoOff_Normal(pUser, pUser->pED->autoInfinitySpiritshot_2[pItem->pII->nCrystalType]);
					}
					pUser->pED->autoInfinitySpiritshot_2[pItem->pII->nCrystalType] = itemId;
				}else
				{
					//1508	1	a,You cannot use that item in a Grand Olympiad Games match.\0	0	79	9B	B0	FF	a,ItemSound3.sys_shortage\0	a,	0	0	0	0	0	a,	a,none\0
					CSystemMessage msg(1508);
			...
			}else if(itemId > 3946 && itemId < 3953)
			{
				if(m_AllowBlessedSpiritShotInOlympiad || !pUser->olympiadUser.IsInside())
				{
					if(pUser->pED->autoInfinitySpiritshot_2[pItem->pII->nCrystalType])
...
void CInfinityShot::AutoOff_Normal(User *pUser)
{
	guard;
	for(UINT n=0;n<6;n++)
	{
		if(pUser->pED->autoInfinitySoulshot_2[n] != 0)
		{
			//1434	1	a,The automatic use of $s1 has been deactivated.\0	0	79	9B	B0	FF	a,	a,	0	0	0	0	0	a,	a,none\0
			CSystemMessage msg(1434);
			msg.AddItem(pUser->pED->autoInfinitySoulshot_2[n]);
			pUser->SendSystemMessage(&msg);
			pUser->pSocket->Send("chdd", 0xFE, 0x12, pUser->pED->autoInfinitySoulshot_2[n], 0);
			pUser->pED->autoInfinitySoulshot_2[n] = 0;
		}
		if(pUser->pED->autoInfinitySpiritshot[n] != 0)
		{
			//1434	1	a,The automatic use of $s1 has been deactivated.\0	0	79	9B	B0	FF	a,	a,	0	0	0	0	0	a,	a,none\0
			CSystemMessage msg(1434);
			msg.AddItem(pUser->pED->autoInfinitySpiritshot_2[n]);
			pUser->SendSystemMessage(&msg);
			pUser->pSocket->Send("chdd", 0xFE, 0x12, pUser->pED->autoInfinitySpiritshot_2[n], 0);
			pUser->pED->autoInfinitySpiritshot_2[n] = 0;
		}
	}
	unguard;
}

bool CInfinityShot::AutoOff_Normal(User *pUser, int itemId)
{
	guard;
	for(UINT n=0;n<6;n++)
	{
		if(pUser->pED->autoInfinitySoulshot_2[n] == itemId)
		{
			pUser->pED->autoInfinitySoulshot_2[n] = 0;
			//1434	1	a,The automatic use of $s1 has been deactivated.\0	0	79	9B	B0	FF	a,	a,	0	0	0	0	0	a,	a,none\0
			CSystemMessage msg(1434);
			...
			unguard;
			return true;
		}else if(pUser->pED->autoInfinitySpiritshot_2[n] == itemId)
		{
			pUser->pED->autoInfinitySpiritshot_2[n] = 0;
			//1434	1	a,The automatic use of $s1 has been deactivated.\0	0	79	9B	B0	FF	a,	a,	0	0	0	0	0	a,	a,none\0
			CSystemMessage msg(1434);
