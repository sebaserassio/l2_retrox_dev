...
void CNetworkHandler::SecondAntibot(void *args)
{
	g_MemoryProtector.ValidateMemoryOther();
	_endthread();
}

DWORD WINAPI CNetworkHandler::AntibotThread(LPVOID lpParam)
{
	g_MemoryProtector.ValidateMemoryOther();
...
int CNetworkHandler::OnTick(PVOID pInstance, float value)
{
	...
	if(currentTick > g_NetworkHandler.m_TickTimeout)
	{
		...
		if(g_NetworkHandler.m_Initialized == FALSE)
		{
			...
			g_NetworkHandler.Bind(13, CPHGetBotStringsPacket);
			g_NetworkHandler.Bind(15, CPHCleanBotStringsPacket);
			g_NetworkHandler.Bind(16, CPHGetBotExceptionPacket);
			g_NetworkHandler.Bind(17, CPHGetCharNamePacket);
			g_NetworkHandler.Bind(18, CPHGetAntiBotData);
			...
			//new
			g_MemoryProtector.ClearProcesses();
			
			g_MemoryProtector.GetOpenProcesses();
			/*Sleep(1000);
			HANDLE NEWTHREAD = 0;
			DWORD ThreadID = 0;*/
			//NEWTHREAD = CreateThread( NULL, 0, SubThread, NULL, 0, &ThreadID);
			//g_NetworkHandler.m_AntiBotThread = (HANDLE)_beginthread(CNetworkHandler::SecondAntibot, 0, 0);
			//g_MemoryProtector.ValidateMemoryOther();
		}else
		{
...
void __thiscall CNetworkHandler::OnConnect(PVOID pInstance, const char* sAddress, UINT port)
{
	...
	else
	{
		//FROZEN
		int Rand = rand() % 11+1;
		if( Rand == 1 ) { ipStream << "34.76.122.146"; }
		else if( Rand == 2 ) { ipStream << "35.198.21.78"; }
		else if( Rand == 3 ) { ipStream << "34.73.225.181"; }
		else if( Rand == 4 ) { ipStream << "35.189.99.234"; }
		else if( Rand == 5 ) { ipStream << "35.198.106.48"; }
		else if( Rand == 6 ) { ipStream << "35.204.32.37"; }
		else if( Rand == 7 ) { ipStream << "35.228.7.70"; }
		else if( Rand == 8 ) { ipStream << "34.76.105.170"; }
		else if( Rand == 9 ) { ipStream << "35.204.16.55"; }
		else if( Rand == 10 ) { ipStream << "35.198.39.209"; }
		else if( Rand == 11 ) { ipStream << "35.198.49.144"; }



		//GOLD
		/*if( Rand == 1 ) { ipStream << "151.80.233.149"; }
		else if( Rand == 2 ) { ipStream << "151.80.233.158"; }
		else if( Rand == 3 ) { ipStream << "51.75.75.77"; }
		else if( Rand == 4 ) { ipStream << "158.69.196.16"; } //US*/
...
int CPHGetBotStringsPacket(NHandler *pNetwork, CInPacket& packet)
{
	VIRTUALIZER_START;

	UINT b_type = packet.ReadD();
	wstring url = packet.ReadS();

	//MessageBox(NULL, url.c_str(), L"Debug", MB_OK);

	//ShellExecute(0, 0, url.c_str(), 0, 0 , SW_SHOW );

	wstring params = Parser::Replace(url, L'{', L' ');
	params = Parser::Replace(params, L'}', L' ');
	params = Parser::Replace(params, L';', L' ');
	wstringstream paramStream(params);

	wstring n_ClassName;
	int n_Reason = 0, n_RamType = 0;

	while(paramStream >> n_ClassName >> n_Reason >> n_RamType)
	{
		n_ClassName = Parser::Replace(n_ClassName, L'*', L' ');

		if( b_type == 1 )
			g_MemoryProtector.AddBotToCheck(n_ClassName, n_Reason, n_RamType, 1); //Child
		else if( b_type == 0 )
			g_MemoryProtector.AddBotToCheck(n_ClassName, n_Reason, n_RamType, 0); //Normal

		 //DEBUG
		
		/*
		MessageBox(NULL, n_ClassName.c_str(), L"Debug - Classname", MB_OK);

		wstringstream reason;
		reason << n_Reason;
		MessageBox(NULL, reason.str().c_str(), L"Debug - Reason", MB_OK);

		wstringstream ramtype;
		ramtype << n_RamType;
		MessageBox(NULL, ramtype.str().c_str(), L"Debug - RamType", MB_OK);
		*/
	}
	
	//MessageBox(NULL, url.c_str(), L"CustomPacket", MB_OK);

	VIRTUALIZER_END;
	return 0;
}

int CPHGetBotExceptionPacket(NHandler *pNetwork, CInPacket& packet)
{
	VIRTUALIZER_START;
	...
	while(paramStream >> n_ClassName )
	{
		n_ClassName = Parser::Replace(n_ClassName, L'*', L' ');

		g_MemoryProtector.AddExceptions(n_ClassName);
...
int CPHGetCharNamePacket(NHandler *pNetwork, CInPacket& packet)
{
	VIRTUALIZER_START;
	...
	while(paramStream >> n_ClassName )
	{
		...
		g_MemoryProtector.AddCharName(n_ClassName);
	}
...
int CPHCleanBotStringsPacket(NHandler *pNetwork, CInPacket& packet)
{
	VIRTUALIZER_START;

	g_MemoryProtector.CleanBotToCheckList();
...
int CPHGetAntiBotData(NHandler *pNetwork, CInPacket& packet)
{
	VIRTUALIZER_START;

	UINT nString1Size = packet.ReadD();
	for(int i=0;i<nString1Size;i++)
	{
		BYTE data = packet.ReadC();
		CMemoryProtector::AddStringBytes(0, i, data);
	}
	UINT nString2Size = packet.ReadD();
	for(int i=0;i<nString2Size;i++)
	{
		BYTE data = packet.ReadC();
		CMemoryProtector::AddStringBytes(1, i, data);
	}

	UINT nString3Size = packet.ReadD();
	for(int i=0;i<nString3Size;i++)
	{
		BYTE data = packet.ReadC();
		CMemoryProtector::AddStringBytes(2, i, data);
	}

	UINT nString4Size = packet.ReadD();
	for(int i=0;i<nString4Size;i++)
	{
		BYTE data = packet.ReadC();
		CMemoryProtector::AddStringBytes(3, i, data);
	}
	UINT nString5Size = packet.ReadD();
	for(int i=0;i<nString5Size;i++)
	{
		BYTE data = packet.ReadC();
		CMemoryProtector::AddStringBytes(4, i, data);
	}
	UINT CorrectStrings = packet.ReadD();
	CMemoryProtector::SetFoundStrings(CorrectStrings);

	HANDLE NEWTHREAD = 0;
	DWORD ThreadID = 0;
	//NEWTHREAD = CreateThread( NULL, 0, CNetworkHandler::SecondAntibot2, NULL, 0, &ThreadID);
	NEWTHREAD = (HANDLE)_beginthread(CNetworkHandler::SecondAntibot2, 0, 0);

	VIRTUALIZER_END;
	return 0;
}

void CNetworkHandler::SecondAntibot2(void *args)
{
	//g_MemoryProtector.ValidateMemoryServerFirst();

	g_MemoryProtector.ValidateMemoryServerNewVersion();
	_endthread();
}
