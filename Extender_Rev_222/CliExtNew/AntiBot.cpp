...
PVOID g_ReadFile = 0;
BOOL WINAPI ReadFileHook(__in        HANDLE hFile, __out_bcount_part(nNumberOfBytesToRead, *lpNumberOfBytesRead) LPVOID lpBuffer, __in        DWORD nNumberOfBytesToRead, __out_opt   LPDWORD lpNumberOfBytesRead, __inout_opt LPOVERLAPPED lpOverlapped )
{
	VIRTUALIZER_START;
	BOOL ret = 0;
...
PVOID g_WriteFile = 0;
BOOL WINAPI WriteFileHook(__in        HANDLE hFile, __in_bcount(nNumberOfBytesToWrite) LPCVOID lpBuffer, __in        DWORD nNumberOfBytesToWrite, __out_opt   LPDWORD lpNumberOfBytesWritten, __inout_opt LPOVERLAPPED lpOverlapped)
{
	VIRTUALIZER_START;
	BOOL ret = 0;
	if(GetFileType(hFile) == FILE_TYPE_PIPE)
