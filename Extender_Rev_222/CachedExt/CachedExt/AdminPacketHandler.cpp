...
void CAdminPacketHandler::Init()
{
	...
	g_AdminPacketHandler[index++] = SendCommandGM;	//121
...
bool __cdecl CAdminPacketHandler::SendCommandGM(CAdminSocket *pSocket, const unsigned char* packet)
{
	guard;
	...
	{
		CHAR buff[8190];
		UINT len = Assemble(buff, sizeof(buff), "cdS", 0x22, CO_SEND_COMMAND_GM, wCommand);
		pServer->Send(len, buff);
		succeed = true;
	}
