...
g_DebugSQLQuery = GetPrivateProfileInt(_T("SQL"), _T("DebugSqlQuery"), 0, File);
	...
	Bind(CI_REQUEST_LOAD_SEMANA_STATUS, RequestLoadSemanaStatus);
...
//	Bind(CI_REQUEST_SAVE_PVPEVENT_PLAYER_REGISTER, RequestSavePvPEventRegister);
	...
	Bind(CI_REQUEST_SAVE_HERO_TIMESTAMP, RequestSaveHeroTime);
	Bind(CI_REQUEST_LOAD_HERO_TIMESTAMP, RequestLoadHeroTime);


	Bind(CI_REQUEST_SAVE_LOGOUT, RequestUpdateCharacterLogout);

	Bind(CI_REQUEST_SAVE_VIP_CHAT_ID, RequestSaveVipChatId);
...
bool PacketHandler::RequestSaveDailyPvP(CQuerySocket* pSocket, const unsigned char* packet)
{
	...
	CQuery query(L"UPDATE [user_data] SET [daily_pvp] = ?, [daily_pvp_timestamp] = ?, [pvp_semana] = [pvp_semana] + 1 WHERE [char_id] = ?");
...
bool PacketHandler::RequestLoadPvPStatus(CQuerySocket* pSocket, const unsigned char* packet)
{
	...
	CQuery query(
		L"SELECT TOP(100) u.[char_id], u.[char_name], u.[class], u.[Lev], u.[duel], "
		L"ISNULL(p.[name], '') AS clan_name, ISNULL(p.[crest_id], 0) AS clan_crest, "
		L"ISNULL(a.[name], '') AS ally_name, ISNULL(a.[crest_id], 0) AS ally_crest "
		L"FROM [user_data] u "
		L"LEFT JOIN [Pledge] p ON u.[pledge_id] = p.[pledge_id] "
		L"LEFT JOIN [Alliance] a ON p.[alliance_id] = a.[id] "
		L"WHERE u.account_id > 0 AND u.builder = 0 "
		L"ORDER BY u.[duel] DESC"
	);
	...
	// Nuevas columnas de Pledge y Alliance
	pConn->BindS(pki.wClanName, sizeof(pki.wClanName));
	pConn->Bind(&pki.crestClan);
	pConn->BindS(pki.wAllyName, sizeof(pki.wAllyName));
	pConn->Bind(&pki.crestAlly);
	...
	{
		PvPInfo& pk = stats[n];
		pSocket->Send("cddSdddSdSd", 0x22, CO_REPLY_LOAD_PVP_STATUS, pk.charId, pk.wName, pk.classId, pk.level, pk.pvpAmount, pk.wClanName, pk.crestClan, pk.wAllyName, pk.crestAlly);
	}
...
//	CQuery query(L"SELECT TOP(100) [char_id], [char_name], [class], [Lev], [daily_pvp] FROM [user_data] WHERE account_id > 0  AND builder = 0 order by [daily_pvp] desc");
	...
	CQuery query(
		L"SELECT TOP(100) u.[char_id], u.[char_name], u.[class], u.[Lev], u.[daily_pvp], "
		L"ISNULL(p.[name], '') AS clan_name, ISNULL(p.[crest_id], 0) AS clan_crest, "
		L"ISNULL(a.[name], '') AS ally_name, ISNULL(a.[crest_id], 0) AS ally_crest "
		L"FROM [user_data] u "
		L"LEFT JOIN [Pledge] p ON u.[pledge_id] = p.[pledge_id] "
	...
	{
		PvPInfo& pk = stats[n];
		pSocket->Send("cddSdddSdSd", 0x22, CO_REPLY_LOAD_DAILY_STATUS, pk.charId, pk.wName, pk.classId, pk.level, pk.pvpAmount, pk.wClanName, pk.crestClan, pk.wAllyName, pk.crestAlly);
	}
...
bool PacketHandler::RequestLoadSemanaStatus(CQuerySocket* pSocket, const unsigned char* packet)
{
	guard;
...
//	CQuery query(L"SELECT TOP(100) [char_id], [char_name], [class], [Lev], [daily_pvp] FROM [user_data] WHERE account_id > 0  AND builder = 0 order by [daily_pvp] desc");
	...
	CQuery query(
		L"SELECT TOP(100) u.[char_id], u.[char_name], u.[class], u.[Lev], u.[pvp_semana], "
		L"ISNULL(p.[name], '') AS clan_name, ISNULL(p.[crest_id], 0) AS clan_crest, "
		L"ISNULL(a.[name], '') AS ally_name, ISNULL(a.[crest_id], 0) AS ally_crest "
		L"FROM [user_data] u "
		L"LEFT JOIN [Pledge] p ON u.[pledge_id] = p.[pledge_id] "
	...
	{
		PvPInfo& pk = stats[n];
		pSocket->Send("cddSdddSdSd", 0x22, CO_REPLY_LOAD_SEMANA_STATUS, pk.charId, pk.wName, pk.classId, pk.level, pk.pvpAmount, pk.wClanName, pk.crestClan, pk.wAllyName, pk.crestAlly);
	}
...
//	CQuery query(L"SELECT TOP(100) [char_id], [char_name], [class], [Lev], [PK] FROM [user_data] WHERE account_id > 0  AND builder = 0 order by [PK] desc");
	...
	CQuery query(
		L"SELECT TOP(100) u.[char_id], u.[char_name], u.[class], u.[Lev], u.[PK], "
		L"ISNULL(p.[name], '') AS clan_name, ISNULL(p.[crest_id], 0) AS clan_crest, "
		L"ISNULL(a.[name], '') AS ally_name, ISNULL(a.[crest_id], 0) AS ally_crest "
		L"FROM [user_data] u "
		L"LEFT JOIN [Pledge] p ON u.[pledge_id] = p.[pledge_id] "
	...
	{
		PKInfo& pk = stats[n];
		pSocket->Send("cddSdddSdSd", 0x22, CO_REPLY_LOAD_PK_STATUS, pk.charId, pk.wName, pk.classId, pk.level, pk.pkAmount, pk.wClanName, pk.crestClan, pk.wAllyName, pk.crestAlly);
	}
...
{
	guard;
	int charId = 0, vipLevel = 0, vipchatId = 0;
	...
	if(charId)
	{
		...
		pDBConn->Bind(&vipchatId);
		if(pDBConn->Execute(L"SELECT vip_level, vip_timestamp, vip_chatId FROM user_data WHERE char_id = %d", charId))
		{
			if(pDBConn->Fetch())
			{
				//done
		...
		pQuerySocket->Send("cddddd", 0x22, CO_REPLY_LOAD_VIP_STATUS, charId, vipLevel, vipTimestamp, vipchatId);
	}
	unguard;
	return false;
}
...
bool PacketHandler::RequestSaveVipChatId(CQuerySocket *pSocket, const unsigned char* packet)
{
	guard;
	UINT charId = 0;

	int vip_chatId = 0;
	Disassemble(packet, "dd", &charId, &vip_chatId);

	CQuery query(L"UPDATE [user_data] SET [vip_chatId] = ? WHERE [char_id] = ?");


	query.AddParam(vip_chatId);
	query.AddParam(charId);
...
bool PacketHandler::RequestSaveHeroTime(CQuerySocket *pSocket, const unsigned char* packet)
{
	guard;
	UINT charId = 0;
	...
	CQuery query(L"UPDATE [user_data] SET [hero_timestamp] = ? WHERE [char_id] = ?");
...
bool PacketHandler::RequestLoadHeroTime(CQuerySocket *pSocket, const unsigned char* packet)
{
	guard;
	...
	CQuery query(L"SELECT [hero_timestamp] FROM [user_data] WHERE [char_id] = ?");
	query.AddParam(charId);
	DBConn *pCon = new DBConn();
	pCon->Bind(&timestamp);
	if(pCon->Execute(query.Get()))
	...
	pSocket->Send("cddd", 0x22, CO_REPLY_CI_REQUEST_LOAD_HERO_TIMESTAMP_EX, charId, timestamp);
