...
#define ReadMem(fAddr,saved,size) clearReadProcessMemory(::GetCurrentProcess(), fAddr, saved, size, nullptr)

#else
#define VirtProtect(fAddr,size,pageAccess,oldProtect) VirtualProtect(fAddr,size,pageAccess,oldProtect)
#define ReadMem(fAddr,saved,size) ReadProcessMemory(::GetCurrentProcess(), fAddr, saved, size, nullptr)
#endif
#pragma endregion 
...
Hook::Hook(wstring const & dll, string const & func, LPVOID addr, bool allowRecalculate):
	...
	flushCache(false)
{
	auto library = ::GetModuleHandle(dll.c_str());
	if(!library) {
